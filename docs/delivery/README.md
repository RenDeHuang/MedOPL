...
后续真实云工作不得继承 slide authority，也不得跳过产品化路线图；Go control-plane MVP takeover、precloud-deployable-rc、local SaaS backend RC 和 local Portal/OPL delivery RC 均只提供本地 RC evidence。下一步仍留在 `real-cloud-authorization-boundary`，下一步先补 repo-native `production-deploy-apply/live` 单一入口，再进入单独授权的 production deploy execution evidence、post-deploy smoke/rollback evidence 和 productionization plan；当前 cursor 不授权新的 Tencent mutation、kubectl、deploy、build/push 或手写 Package D execution。
...
-> next gap is repo-native `production-deploy-apply/live` single entrypoint; do not hand-run kubectl apply
