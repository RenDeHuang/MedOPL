...
-> separately authorize repo-native run-scoped platform runner Job preflight with unique `medopl-platform-runner-preflight-<runid>` name, `node.tke.cloud.tencent.com/machineset=np-6l4nkdto`, and real `PACKAGE_D_RUNNER_IMAGE_REF`; observe/evidence/cleanup only that Job
