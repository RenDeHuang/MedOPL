...
当前 product cursor 是 `goal-owner-created-or-approved-account-launch-approval-decision`，状态是 G4 authorized production-canary path evidence complete / launch approval decision required。MedOPL 的当前 delivery truth 是：平台给 OPL-Webui 登录用户在当前 workspace 提供托管 runtime、文件空间、计费、审计和释放；MedOPL 不承担 OPL 自身科研能力。最新 product RC closeout 是 `feat/v22-local-product-rc-single-flow` / `4f195820dcb351030cb1064172bfec2062abc279`。`goal-g4-upload-file-billing-event-writeback-fix` / `0f7c80b1c3062ddc33e612b37cdeeb9c7e483be1` 已修复 upload_file billing event writeback，保持 business truth、billing/audit fail-closed、redacted diagnostic receipt 和重复 upload/idempotency 语义。Release Image run `28286560942` built image tag `0f7c80b`; Cloud Rollout production_launch run `28286621467` passed deploy receipt、OPL-Webui consumer canary、Portal public entry、HTTP-to-HTTPS redirect、Go health/ready availability、soak、concurrency pressure、rollback drill、continuous canary、alerting、final release decision、production receipt manifest、`verify:cloud-release-candidate` 和 `verify:production-complete-candidate`。Redacted receipt manifest hash is `sha256:16fb4289f3dd91543f083b1c1ad773e44be6d5a74f76a56b417b39974bca4612`。Cloud Rollout `workflow_dispatch` inputs and the GitHub `production` environment approval are the production launch operations safety gate; this is not runtime gate、充值、套餐、余额、quota 或 billing truth。下一步只决定是否开放 scoped production-canary path 给 owner-created-or-approved MedOPL accounts；这不是 external PSP settlement、unrestricted full production、多区域、SLA、企业合规、持续授权、未观测租户/资源或长时间稳定性。
