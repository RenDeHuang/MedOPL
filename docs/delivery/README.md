...
当前 product cursor 是 `opl-webui-runtime-production-slice`，状态是 cloud release candidate evidence present / before production-complete claim。MedOPL 的当前 delivery truth 是：平台给 OPL-Webui 登录用户在当前 workspace 提供托管 runtime、文件空间、计费、审计和释放；MedOPL 不承担 OPL 自身科研能力。最新 product RC closeout 是 `feat/v22-local-product-rc-single-flow` / `4f195820dcb351030cb1064172bfec2062abc279`。当前 trunk `874bd20fc6fef9f9268ad2b058489d7dd7cc37de` 已把 Portal public entry、HTTP-to-HTTPS redirect、Go health/ready availability 和 Cloud Rollout probe 纳入当前 cloud RC gate；生产 claim 仍受 root/frontend 生产依赖 audit、浏览器可访问性回归、release owner readiness、observability / production deploy receipt、S-level UI polish gate、production-complete owner gate 和 canary proof 约束。当前授权 canary path 的 cloud release candidate evidence 来自 Release Image run `28012858200` 和 Cloud Rollout run `28013437604`：Release Image built tag `8753e2b`，Cloud Rollout passed rollout apply、Portal public entry、HTTP redirect、Go health/ready availability、OPL-Webui consumer canary、production receipt manifest 和 `verify:cloud-release-candidate`。Redacted manifest 位于 GitHub Actions artifact / `.runtime` pointer，包含七类 owner receipts、12 个 lifecycle sections、`claim=cloud_release_candidate`、`state=complete`，sha256 `3d2b6e8ffe3601ccc3f0489e59079657af0fe2f9972cf9f34713d170903cd85b`。这不是 production complete，不证明多区域、SLA、企业合规、持续授权、未观测租户/资源或长时间稳定性。
