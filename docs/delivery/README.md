...
当前 product cursor 是 `real-cloud-authorization-boundary`，状态是 authorization-required / before real-cloud readiness。pre-cloud、本地 RC、Package C live canary、Package D production deploy apply 和 Package D in-cluster HTTP reachability 已各自形成 evidence，但它们不等于正式稳定上线。最新 landed repo closeout 是 `recovery/platform-v22-trunk` / `e8a3a645bc24a92fdb8288ee8a6925db6466945d`。当前 delivery truth 采用 `goal-current.json` 的 Production Launch Goal / Gap Map 作为机器 cursor 派生：目标是 `platform-provisioned / customer-dedicated` 的多租户 SaaS 托管 OPL 工作台，Portal 是用户/管理员入口，平台代管云资源，workspace lifecycle 通过 ResourceBinding / CloudOperation ledger、billing、audit、quota、rollback、cleanup 和 redacted evidence 闭合。Package D 现在是已部署的 TKE platform-service baseline：`pdrun-20260616-004` 完成四服务 deploy/rollout，`psr-20260617-004` 证明四个 ClusterIP Service HTTP `200` 且 smoke Job 清理成功。Production launch Gap 01-08c 已收口为 repo-native contract/local gates 或云端 server-side dry-run evidence。Gap 08d 真实 apply 仍未执行：`gap08d-qcloud-ingress-real-001` 因缺 repo-native real mutation runner fail-closed，`gap08d-qcloud-ingress-real-002` 因 runner env gate 设计错误 fail-closed，`gap08d-qcloud-ingress-real-003` 因 `dry_run_qcloud_cert_secret` 引用尚未生成的 `.runtime/.../qcloud-cert-secret-redacted.json` fail-closed；第三次 evidence 为 `.runtime/package-d-external-access-strategy/gap08d-qcloud-ingress-real-003/real-mutation-redacted.json`，`medopl-portal-tls` 与 Portal Ingress 均仍为 `NotFound`，external access env allowlist 与 process `RUN_TENCENT_DEPLOY_EXECUTION=external-access` 已通过，`TENCENT_SSL_CERT_ID` 未泄露，且无越界 mutation。Gap 08d-runner-dryrun-manifest-file-fix 已修正 `tests/support/cloud-prework/package-d-external-access-runner.js`：`qcloud-ingress-apply` 在 dry-run 前写出 `qcloud-cert-secret-redacted.json` 与 `portal-ingress-redacted.json` redacted evidence，并对 live qcloud Secret/Ingress manifest 使用 stdin server-side dry-run/apply，不再依赖 redacted manifest 文件路径；`package-d-external-access.env` 仍只允许五个业务 key：`PORTAL_HOST_DOMAIN`、`INGRESS_CLASS`、`TLS_SECRET_NAME`、`TENCENT_SSL_CERT_ID`、`EXTERNAL_SMOKE_URL`。qcloud 证书 Secret 仍为 `Opaque` + `stringData.qcloud_cert_id`，`TENCENT_SSL_CERT_ID` 原文不得进入 docs/evidence/logs。当前唯一 gap 仍是 `production-launch-gap-08d-qcloud-opaque-cert-id-secret-ingress-real-apply`：云端以 run-id `gap08d-qcloud-ingress-real-004` 执行 repo-native apply runner，且该 gap 之前仍不授权 DB password、Portal admin password、Tencent SecretId/SecretKey、TLS private key/cert material、Deployment/Service 修改、DNS 修改、LoadBalancer Service、Package C/tenant resources、build/push、deploy/rollout/rollback、Package C live 或 public access completion claim。
...
后续真实云工作不得继承 slide authority，也不得跳过产品化路线图；Go control-plane MVP takeover、precloud-deployable-rc、local SaaS backend RC 和 local Portal/OPL delivery RC 均只提供本地 RC evidence。下一步仍留在 `real-cloud-authorization-boundary`，但当前唯一 gap 已从 qcloud TLS Secret + Ingress server-side dry-run 和 fail-closed 08d attempts 推进到 repo-native 08d real apply runner dry-run manifest file fix。Package D 已 deployed inside TKE 且 in-cluster HTTP reachability passed；external/public user access not yet exposed 仍是事实，下一步只允许在单独授权下创建/应用 qcloud `Opaque` Secret `medopl-portal-tls` 与 Portal qcloud Ingress，并只做 get/describe verification、DNS post-apply validation 与 HTTPS smoke；不允许修改 Deployment/Service、DNS、LoadBalancer Service、Package C/tenant resources，不声明公网可用。当前 cursor 不授权新的 Tencent API mutation、deploy、build/push、Package C live、production PostgreSQL write/read 或手写 Package D execution。
...
-> Gap 08d real apply attempts remain fail-closed with no real mutation; latest cloud attempt `gap08d-qcloud-ingress-real-003` failed at `dry_run_qcloud_cert_secret` because a redacted manifest file was referenced before it existed
-> repo-native qcloud external access runner now writes redacted qcloud Secret/Portal Ingress manifest evidence before dry-run and uses live stdin manifests for dry-run/apply; next authorized run-id is `gap08d-qcloud-ingress-real-004`
-> authorized deploy
