...
当前 product cursor 是 `real-cloud-authorization-boundary`，状态是 authorization-required / before real-cloud readiness。pre-cloud、本地 RC、Package C live canary、Package D production deploy apply 和 Package D in-cluster HTTP reachability 已各自形成 evidence，但它们不等于正式稳定上线。最新 landed repo closeout 是 `recovery/platform-v22-trunk` / `96aac636d868d6e3da4bccea31e94476a4b38c80`。当前 delivery truth 采用 `goal-current.json` 的 Production Launch Goal / Gap Map 作为机器 cursor 派生：目标是 `platform-provisioned / customer-dedicated` 的多租户 SaaS 托管 OPL 工作台，Portal 是用户/管理员入口，平台代管云资源，workspace lifecycle 通过 ResourceBinding / CloudOperation ledger、billing、audit、quota、rollback、cleanup 和 redacted evidence 闭合。Package D 现在是已部署的 TKE platform-service baseline：`pdrun-20260616-004` 完成四服务 deploy/rollout，`psr-20260617-004` 证明四个 ClusterIP Service HTTP `200` 且 smoke Job 清理成功。Production launch Gap 01-08n-prep 已收口为 repo-native contract/local gates、云端 server-side dry-run evidence、云端 bounded mutation evidence、只读 diagnostics evidence 或 shared-edge strategy gate。Portal external access 仍未完成：`portal-frontend-edge` NodePort direct HTTP `200`，DNS/TLS/Ingress Ready 均已到位，qcloud 控制台 backend `10.66.0.42:30336` healthy，但失败 Portal CLB `lb-b33auprw` 仍返回 stgw `504`；working `opl.medopl.cn` 仍在 `lb-lhj3bgii` 返回 HTTPS `200`。继续修 `lb-b33auprw` 收益低，直接复用 OPL-Webui CLB 会混合历史 OPL-Webui 与 MedOPL Portal edge 责任。当前唯一 gap 是 `production-launch-gap-08n-1-shared-edge-dry-run-creation-strategy`：未来只允许 shared edge CLB dry-run/designation strategy、route attach plan、DNS cutover plan、rollback/smoke/evidence plan；不授权 DB password、Portal admin password、Tencent SecretId/SecretKey、TLS private key/cert material、DNS/CLB/Ingress/Service/Deployment mutation、LoadBalancer Service、Package C/tenant resources、build/push、deploy/rollout/rollback、Package C live、compatibility layer、fallback route 或 public access completion claim。
...
后续真实云工作不得继承 slide authority，也不得跳过产品化路线图；Go control-plane MVP takeover、precloud-deployable-rc、local SaaS backend RC 和 local Portal/OPL delivery RC 均只提供本地 RC evidence。下一步仍留在 `real-cloud-authorization-boundary`，但当前唯一 gap 已推进到 Gap 08n-1 shared edge dry-run / creation strategy。Package D 已 deployed inside TKE 且 in-cluster HTTP reachability passed；external/public user access not yet complete 仍是事实。下一步只允许 repo-native strategy pack 规划/验证 MedOPL shared edge CLB ownership、host routing、TLS/WAF/security/monitoring boundary、DNS cutover、rollback、Portal/OPL smoke 和 failed `lb-b33auprw` cleanup gap；不允许修旧 LB 变成长期入口，不允许复用 OPL-Webui CLB 作为隐藏 fallback，不允许长期双 CLB、双写 route 或 fallback host，不允许修改 Deployment、现有 `Service/portal-frontend` ClusterIP、`Service/portal-frontend-edge`、DNS、CLB、Ingress、LoadBalancer Service、Package C/tenant resources，不声明公网可用。当前 cursor 不授权新的 Tencent API mutation、deploy、build/push、Package C live、production PostgreSQL write/read 或手写 Package D execution。
