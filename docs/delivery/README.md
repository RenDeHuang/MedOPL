...
后续真实云工作不得继承 slide authority，也不得跳过产品化路线图；Go control-plane MVP takeover、precloud-deployable-rc、local SaaS backend RC 和 local Portal/OPL delivery RC 均只提供本地 RC evidence。下一步仍留在 `real-cloud-authorization-boundary`，只推进 productionization plan：authorized PostgreSQL prepare/live execution、billing/audit ledger、Portal 开通入口、workspace storage quota；当前 cursor 不授权新的 DB secret read、Tencent mutation、kubectl、deploy、build/push 或 Package D。
...
-> productionize PostgreSQL write wiring / billing-audit ledger / Portal opening entry / workspace storage quota
-> authorized deploy
