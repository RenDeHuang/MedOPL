...
当前 product cursor 是 `opl-webui-runtime-production-slice`，状态是 cloud release candidate evidence present / production-complete candidate evidence present / before final production-complete release decision。MedOPL 的当前 delivery truth 是：平台给 OPL-Webui 登录用户在当前 workspace 提供托管 runtime、文件空间、计费、审计和释放；MedOPL 不承担 OPL 自身科研能力。最新 product RC closeout 是 `feat/v22-local-product-rc-single-flow` / `4f195820dcb351030cb1064172bfec2062abc279`。当前 trunk `44941cf93cb3ee9b85a2ae5069d65333e07162f6` 已把 Portal public entry、HTTP-to-HTTPS redirect、Go health/ready availability、Cloud Rollout probe 和 Phase A live DB persistence proof 纳入当前 cloud gate；生产 claim 仍受 root/frontend 生产依赖 audit、浏览器可访问性回归、release owner readiness、observability / production deploy receipt、S-level UI polish gate、production-complete owner gate 和 canary proof 约束。当前授权 canary path 的 evidence 来自 Release Image run `28049187862` 和 Cloud Rollout run `28049216178`：Release Image built tag `44941cf`，Cloud Rollout passed production DB auth/secret sync、runtime/storage/billing-audit owner receipt lanes、kubectl rollout、deploy receipt、Portal public entry、HTTP redirect、Go health/ready availability、OPL-Webui consumer canary、production receipt manifest、`verify:cloud-release-candidate` 和 `verify:production-complete-candidate`。Redacted manifest 位于 GitHub Actions artifact / `.runtime` pointer，包含七类 owner receipts、12 个 lifecycle sections、八个 production-complete criteria including `business_db_persistence_receipt`、`claim=production_complete`、`state=complete`，sha256 `a8630806f231643e0c95ec113ae6c1c2b371a20f2ed4ed80873f81de2f0b1976`。这不是最终 production complete release decision，不证明多区域、SLA、企业合规、持续授权、未观测租户/资源或长时间稳定性。
