...
当前 product cursor 是 `opl-webui-runtime-production-slice`，状态是 cloud release candidate evidence present / before production-complete claim。MedOPL 的当前 delivery truth 是：平台给 OPL-Webui 登录用户在当前 workspace 提供托管 runtime、文件空间、计费、审计和释放；MedOPL 不承担 OPL 自身科研能力。最新 product RC closeout 是 `feat/v22-local-product-rc-single-flow` / `4f195820dcb351030cb1064172bfec2062abc279`。当前 trunk `545e2cfa303d13de2685024b86d438854b57ba15` 通过 GitHub v22 verify run `27974444437`，并保持 Portal UI/UX production gates：root/frontend 生产依赖 audit、浏览器可访问性回归、release owner readiness、observability / production deploy receipt、S-level UI polish gate、production-complete owner gate 和 canary proof 都必须在 production claim 前满足。当前授权 canary path 的 cloud release candidate evidence 来自 Release Image run `27972408990` 和 Cloud Rollout run `27972458642`：Release Image built tag `485afcf`，Cloud Rollout passed rollout apply、deploy receipt、OPL-Webui consumer canary、availability probe、production receipt manifest 和 `verify:cloud-release-candidate`。红acted manifest 位于 GitHub Actions artifact / `.runtime` pointer，包含七类 owner receipts、12 个 lifecycle sections、`claim=cloud_release_candidate`、`state=complete`，sha256 `43d6e9de0915190b552e3d0bcf5f9b6fd319953c986b4ad2e998c4c97f976ad2`。这不是 production complete，不证明多区域、SLA、企业合规、持续授权或未观测租户/资源。
