...
后续真实云工作不得继承 slide authority，也不得跳过产品化路线图；Go control-plane MVP takeover、precloud-deployable-rc、local SaaS backend RC 和 local Portal/OPL delivery RC 均只提供本地 RC evidence。下一步仍留在 `real-cloud-authorization-boundary`：Package D `production-deploy-apply` run `pdrun-20260616-004` 已让四个服务在 TKE 内 ready，但当前 Service 仍是 ClusterIP-only；readonly service reachability runs `psr-20260617-001` 和 `psr-20260617-002` 均在 `smoke_job_wait_complete` 超时并按策略清理 Job，第三次 `psr-20260617-003` 已定位到 curl 容器 `CreateContainerConfigError`：`curlimages/curl:8.8.0` 使用非数字用户 `curl_user`，与 `runAsNonRoot: true` 组合时 Kubernetes 无法验证非 root。本轮只修 repo-native smoke Job manifest 的数字非 root curl container securityContext，并保持 fail-fast curl / diagnostics。下一步是授权使用 `psr-20260617-004` 重新执行 readonly service reachability / in-cluster HTTP smoke runner，以及 Portal external access strategy（Ingress / LoadBalancer / internal gateway / admin-only port-forward）选择与授权边界。当前 cursor 不授权新的 Tencent mutation、kubectl、deploy、build/push、Ingress/LB 变更或手写 Package D execution。
...
-> next gap is authorized rerun `psr-20260617-004` of that service reachability runner plus Portal external access strategy; choose Ingress, LoadBalancer, internal gateway or admin-only port-forward under separate authorization before any public/user access claim
