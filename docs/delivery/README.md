...
当前 product cursor 是 `real-cloud-authorization-boundary`，状态是 authorization-required / before real-cloud readiness。pre-cloud、本地 RC、Package C live canary、Package D production deploy apply 和 Package D in-cluster HTTP reachability 已各自形成 evidence，但它们不等于正式稳定上线。最新 landed repo closeout 是 `recovery/platform-v22-trunk` / `55484701a515d433aeb7fcb43dbd057153eb41ea`。当前 delivery truth 采用 `goal-current.json` 的 Production Launch Goal / Gap Map 作为机器 cursor 派生：目标是 `platform-provisioned / customer-dedicated` 的多租户 SaaS 托管 OPL 工作台，Portal 是用户/管理员入口，平台代管云资源，workspace lifecycle 通过 ResourceBinding / CloudOperation ledger、billing、audit、quota、rollback、cleanup 和 redacted evidence 闭合。Package D 现在是已部署的 TKE platform-service baseline：`pdrun-20260616-004` 完成四服务 deploy/rollout，`psr-20260617-004` 证明四个 ClusterIP Service HTTP `200` 且 smoke Job 清理成功。Production launch Gap 01-08b 已收口为 repo-native contract/local gates；Gap 08c qcloud TLS Secret and Ingress server-side dry-run 已在云端通过，evidence 为 `.runtime/package-d-external-access-strategy/gap08c-qcloud-ingress-dry-run-001/dry-run-redacted.json`。Dry-run 确认 `medopl-platform`、`portal-frontend` ClusterIP Service `8080/TCP`、IngressClass `qcloud` / controller `cloud.tencent.com/ingress-controller`、TLS Secret manifest shape、TLS Secret server-side dry-run / validation、Ingress manifest shape 和 Ingress server-side dry-run 均通过；`medopl-portal-tls` 与 Portal Ingress 均未真实创建并确认 `NotFound`；无真实 mutation，redaction audit 通过。第一次 Gap 08d 真实 mutation 在云端 fail-closed，evidence 为 `.runtime/package-d-external-access-strategy/gap08d-qcloud-ingress-real-001/real-mutation-redacted.json`：当时缺 repo-native real mutation runner、缺 5 个 external access env keys、未授权 TLS material path，且 Secret/Ingress 仍为 `NotFound`。Gap 08d-prep 现在补齐 `tests/support/cloud-prework/package-d-external-access-runner.js`，支持 `qcloud-ingress-dry-run` / `qcloud-ingress-apply`，并将 qcloud 证书 Secret 修正为 `Opaque` + `stringData.qcloud_cert_id`；`TENCENT_SSL_CERT_ID` 只允许作为授权 env key 存在，原文不得进入 docs/evidence/logs。当前唯一 gap 是 `production-launch-gap-08d-qcloud-opaque-cert-id-secret-ingress-real-apply`：云端必须先把 `PORTAL_HOST_DOMAIN`、`INGRESS_CLASS`、`TLS_SECRET_NAME`、`TENCENT_SSL_CERT_ID`、`EXTERNAL_SMOKE_URL` 放入授权 env 文件或单独 allowlist env 文件，再以 `RUN_TENCENT_DEPLOY_EXECUTION=external-access` 执行 repo-native apply runner。该 gap 之前仍不授权 DB password、Portal admin password、Tencent SecretId/SecretKey、TLS private key/cert material、Deployment/Service 修改、DNS 修改、LoadBalancer Service、Package C/tenant resources、build/push、deploy/rollout/rollback、Package C live 或 public access completion claim。
...
-> next unique gap is `production-launch-gap-07-external-access-strategy-contract`: Portal external access strategy contract after minimum SaaS closure, without default public exposure
...
-> then productionize canary, rollback/cleanup and redaction evidence before external access
-> authorized deploy
