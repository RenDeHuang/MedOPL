...
当前 product cursor 是 `goal-g4-upload-file-billing-event-writeback-fix`，状态是 G4 production_launch OPL-Webui consumer canary 已由 `1e33b38` diagnostic receipt 收敛到 `upload_file -> save_billing_event` 写入失败。MedOPL 的当前 delivery truth 是：平台给 OPL-Webui 登录用户在当前 workspace 提供托管 runtime、文件空间、计费、审计和释放；MedOPL 不承担 OPL 自身科研能力。最新 product RC closeout 是 `feat/v22-local-product-rc-single-flow` / `4f195820dcb351030cb1064172bfec2062abc279`。当前 slice 必须修复 upload_file billing event writeback，同时保持 business truth、billing/audit fail-closed、redacted diagnostic receipt 和重复 upload/idempotency 语义。G4 仍必须在修复后证明当前 trunk/image 已部署或执行 release image + rollout、Portal public entry、HTTP-to-HTTPS redirect、Go health/ready availability、OPL-Webui consumer entry、G3 evidence restore、rollback readiness、monitoring/alerting owner 和 cost ceiling as account balance / plan / quota。Cloud Rollout `workflow_dispatch` inputs and the GitHub `production` environment approval are the production launch operations safety gate; `MEDOPL_PRODUCTION_*` GitHub vars/secrets are not required and this is not runtime gate、充值、套餐、余额、quota 或 billing truth。这不是最终 production complete release decision，不证明多区域、SLA、企业合规、持续授权、未观测租户/资源或长时间稳定性。
