...
-> publish the fixed Package D runner image tag through a private build runner using only `TCR_ID`, `TCR_SECRET` and `PACKAGE_D_RUNNER_IMAGE_REF` from `package-d-deploy.env`; no GitHub Actions secrets current path, no deploy and no TKE/VPC runner Docker build/push
