...
后续真实云工作不得继承 slide authority，也不得跳过产品化路线图；Go control-plane MVP takeover、precloud-deployable-rc、local SaaS backend RC 和 local Portal/OPL delivery RC 均只提供本地 RC evidence。下一步仍留在 `real-cloud-authorization-boundary`，但当前唯一 gap 已从 qcloud TLS Secret + Ingress server-side dry-run 推进到真实创建授权包。Package D 已 deployed inside TKE 且 in-cluster HTTP reachability passed；external/public user access not yet exposed 仍是事实，下一步只允许在单独授权下创建/应用 `Secret/medopl-portal-tls` 和 Portal qcloud Ingress，并只做 get/describe verification、DNS post-apply validation 与 HTTPS smoke；不允许修改 Deployment/Service、DNS、LoadBalancer Service、Package C/tenant resources，不声明公网可用。当前 cursor 不授权新的 Tencent mutation、deploy、build/push、Package C live、production PostgreSQL write/read 或手写 Package D execution。
