...
Goal delivery and Commercial Launch journeys are now explicitly linked: `tests/fixtures/v22/goal-current.json#/goal_lifecycle/current/affected_journeys` and `#/current_leaf/affected_journeys` must list only journey ids from `contracts/medopl-commercial-launch-product-contract-matrix.json`. The delivery rule is `Goal -> affected journey ids -> implementation/evidence -> goal closeout`; journey ids do not replace goals, and goals cannot bypass product journey truth for product-facing work.
