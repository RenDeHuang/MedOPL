...
当前 product cursor 是 `real-cloud-authorization-boundary`，状态是 authorization-required / before real-cloud readiness。pre-cloud、本地 RC、Package C live canary、Package D production deploy apply 和 Package D in-cluster HTTP reachability 已各自形成 evidence，但它们不等于正式稳定上线。最新 landed repo closeout 是 `recovery/platform-v22-trunk` / `2be51442035dee610283eba89b5b216c49e91607`。当前 delivery truth 采用 `goal-current.json` 的 Production Launch Goal / Gap Map 作为机器 cursor 派生：目标是 `platform-provisioned / customer-dedicated` 的多租户 SaaS 托管 OPL 工作台，Portal 是用户/管理员入口，平台代管云资源，workspace lifecycle 通过 ResourceBinding / CloudOperation ledger、billing、audit、quota、rollback、cleanup 和 redacted evidence 闭合。Package D 现在是已部署的 TKE platform-service baseline：`pdrun-20260616-004` 完成四服务 deploy/rollout，`psr-20260617-004` 证明四个 ClusterIP Service HTTP `200` 且 smoke Job 清理成功。Production launch Gap 01-07 已收口为 repo-native contract/local gates；Gap 08a external access prerequisites discovery 已在云端完成，evidence 为 `.runtime/package-d-external-access-strategy/gap08a-prereq-discovery-001/prereq-discovery-redacted.json`，确认 `medopl-platform`、`portal-frontend` Service `172.21.5.91:8080`、IngressClass `qcloud` / controller `cloud.tencent.com/ingress-controller`、cert-manager 不可用且 `medopl-portal-tls` 不存在。Gap 08b qcloud Ingress + TLS Secret material readiness 已通过 `package-d-external-access-strategy-runner.js --mode qcloud-tls-readiness-contract-local-gate` 收口，固定 `PORTAL_HOST_DOMAIN=portal.medopl.cn`、`INGRESS_CLASS=qcloud`、`TLS_SECRET_NAME=medopl-portal-tls`、`EXTERNAL_SMOKE_URL=https://portal.medopl.cn/`，推荐腾讯云 SSL 证书或人工签发证书材料通过授权路径创建 Kubernetes TLS Secret，并明确 TLS cert/key 不得进入 git、docs、runtime redacted evidence 或 logs。当前唯一 gap 是 `production-launch-gap-08c-qcloud-ingress-tls-server-side-dry-run`：先 TLS Secret creation server-side dry-run 或 manifest validation，再 Ingress server-side dry-run，不执行真实 mutation。该 gap 之前仍不授权 secret、kubeconfig、DB password、TLS private key、kubectl apply、deploy、build/push、Tencent mutation、Package C live、production PostgreSQL write/read、Ingress/LB/DNS/TLS mutation 或 public access completion claim。
...
后续真实云工作不得继承 slide authority，也不得跳过产品化路线图；Go control-plane MVP takeover、precloud-deployable-rc、local SaaS backend RC 和 local Portal/OPL delivery RC 均只提供本地 RC evidence。下一步仍留在 `real-cloud-authorization-boundary`，但当前唯一 gap 已从 Portal external access strategy contract 推进到 qcloud TLS Secret + Ingress server-side dry-run authorization。Package D 已 deployed inside TKE 且 in-cluster HTTP reachability passed；external/public user access not yet exposed 仍是事实，下一步只允许针对 `portal.medopl.cn` / `qcloud` / `medopl-portal-tls` 做单独授权的 TLS Secret manifest validation 或 server-side dry-run，然后 Ingress server-side dry-run，不执行 mutation，不声明公网可用。当前 cursor 不授权新的 Tencent mutation、kubectl apply、deploy、build/push、Ingress/LB/HTTPS 变更、Package C live、production PostgreSQL write/read 或手写 Package D execution。
