...
当前 product cursor 是 `real-cloud-authorization-boundary`，状态是 authorization-required / before real-cloud readiness。pre-cloud、本地 RC、Package C live canary、Package D production deploy apply 和 Package D in-cluster HTTP reachability 已各自形成 evidence，但它们不等于正式稳定上线。最新 landed repo closeout 是 `feat/opl-style-development-discipline-convergence` / `138d18e8d3df72bb615fac9da561e77189f2effe`。当前 delivery truth 采用 `goal-current.json` 的 Production Launch Goal / Gap Map 作为机器 cursor 派生：目标是 `platform-provisioned / customer-dedicated` 的多租户 SaaS 托管 OPL 工作台，Portal 是用户/管理员入口，平台代管云资源，workspace lifecycle 通过 ResourceBinding / CloudOperation ledger、billing、audit、quota、rollback、cleanup 和 redacted evidence 闭合。Package D 现在是已部署的 TKE platform-service baseline：`pdrun-20260616-004` 完成四服务 deploy/rollout，`psr-20260617-004` 证明四个 ClusterIP Service HTTP `200` 且 smoke Job 清理成功。Production launch Gap 01-08o-prep 已收口为 repo-native contract/local gates、云端 server-side dry-run evidence、云端 bounded mutation evidence、只读 diagnostics evidence 或 strategy gate。Portal external access 仍未完成：NodePort direct 和 CLB-like header matrix 都是 HTTP `200`，Portal Pod access logs 记录 NodePort 请求 `200`，CLB healthcheck 到 Pod `HEAD /` 返回 `200`，但 current Portal CLB `lb-pwv9zgky` 仍 HTTPS `504` stgw；working `opl.medopl.cn` 仍在 `lb-lhj3bgii` 返回 HTTPS `200`。Gap 08o run `gap08o-tencent-clb-readonly-diagnostics-003` authenticated to Tencent API and confirmed Portal/OPL CLBs, HTTPS listeners and registered targets, but rule, target-health and security-group diagnostics hit runner/API TypeError or unsupported shape and made `rule_missing` untrustworthy because working OPL was also marked missing. 当前唯一 gap 是 `production-launch-gap-08o-runner-fix`：修复 repo-native Tencent CLB readonly diagnostics runner 的 SDK calls、equivalent readonly fallbacks、unsupported classification and Portal-vs-OPL diff-based root cause；不授权 DB password、Portal admin password、Tencent SecretId/SecretKey 输出、TLS private key/cert material、DNS/CLB/Kubernetes mutation、Ingress/Service/Deployment mutation、LoadBalancer Service、Package C/tenant resources、build/push、deploy/rollout/rollback、Package C live 或 public access completion claim。
...
后续真实云工作不得继承 slide authority，也不得跳过产品化路线图；Go control-plane MVP takeover、precloud-deployable-rc、local SaaS backend RC 和 local Portal/OPL delivery RC 均只提供本地 RC evidence。下一步仍留在 `real-cloud-authorization-boundary`，但当前唯一 gap 已推进到 Gap 08o runner fix after readonly diagnostics. Package D 已 deployed inside TKE 且 in-cluster HTTP reachability passed；external/public user access not yet complete 仍是事实。下一步只允许 repo-native runner 对 current Portal CLB `lb-pwv9zgky`、previous Portal CLB `lb-b33auprw` if discoverable 和 working OPL CLB `lb-lhj3bgii` 做只读对比，分类 `listener_missing`、`rule_missing`、`target_missing`、`target_unhealthy`、`wrong_target_port`、`wrong_target_ip`、`security_group_block`、`source_ip_passthrough_mismatch`、`listener_rule_mismatch`、`incomplete_readonly_diagnosis` 或 `unknown_clb_data_plane_504`；如果 working OPL 也缺 rule evidence，则不得输出 trustworthy `rule_missing`。当前 cursor 不授权 Tencent mutation、DNS/CLB/Kubernetes mutation、kubectl、deploy、build/push、Package C live、production PostgreSQL write/read 或 public access completion claim。
...
-> Gap 08n shared edge CLB strategy/local gate landed locally after Portal still returned stgw `504` through `lb-b33auprw` despite NodePort direct HTTP `200`, DNS/TLS/Ingress Ready and console healthy backend evidence; it compares A continue fixing `lb-b33auprw`, B reuse working OPL-Webui CLB `lb-lhj3bgii`, and C create/designate a MedOPL shared edge CLB; recommends C for formal production stability, keeps current OPL-Webui entry non-disruptive, forbids compatibility layer/fallback host/long-term dual CLB/dual-write route, requires a later explicit cleanup gap for failed Portal CLB resources, and writes `.runtime/package-d-external-access-strategy/<runid>/shared-edge-strategy-redacted.json`
-> Gap 08o Tencent CLB readonly diagnostics runner/local gate landed locally after operator diagnostics showed Portal NodePort direct and CLB-like headers return `200`, Pod logs record those requests as `200`, qcloud CLB health checks reach the Pod and return `200`, but current Portal CLB `lb-pwv9zgky` still returns HTTPS `504` while working OPL-Webui CLB `lb-lhj3bgii` returns HTTPS `200`; the runner single entrypoint is `node tests/support/cloud-prework/tencent-clb-readonly-diagnostics-runner.js --env /home/dev/.secrets/medopl/v22/tencent-clb-readonly.env --run-id <runid> --authorized 1`, the env allowlist is limited to Tencent Cloud credentials, region, Portal/OPL CLB IDs, host domains and expected NodePort targets, allowed APIs are CLB Describe* readonly calls only, mutation prefixes `Create/Modify/Delete/Register/Deregister/Attach/Detach/Set/Associate/Disassociate/Rewrite/Batch` are rejected, and evidence is `.runtime/package-d-external-access-strategy/<runid>/tencent-clb-readonly-diagnostics-redacted.json`
-> Gap 08o readonly cloud run `gap08o-tencent-clb-readonly-diagnostics-003` authenticated to Tencent API and confirmed Portal CLB `lb-pwv9zgky`, OPL CLB `lb-lhj3bgii`, HTTPS:443 listeners and registered targets `eks-pnrndv6u:30336` / `eks-pnrndv6u:32258`, but rule, target-health and security-group diagnostics exposed runner/API shape gaps: current Tencent Node SDK lacks `DescribeRules`, `DescribeTargetsHealth` is actually `DescribeTargetHealth`, security-group detail must use supported readonly equivalent data, unsupported customized config must be non-fatal, and OPL working rule evidence prevents trustworthy `rule_missing` until rules parse correctly. Evidence is `.runtime/package-d-external-access-strategy/gap08o-tencent-clb-readonly-diagnostics-003/tencent-clb-readonly-diagnostics-redacted.json`; current local work fixes the runner and prepares rerun `gap08o-tencent-clb-readonly-diagnostics-004`.
-> authorized deploy
