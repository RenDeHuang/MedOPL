...
-> publish the fixed Package D runner image tag through public-repo-safe GitHub Actions `workflow_dispatch` with environment `package-d-image-publish`, required reviewer, branch restriction to `recovery/platform-v22-trunk` / `release/*`, environment secrets only `TCR_ID` / `TCR_SECRET`, no repository secrets and no automatic triggers; no deploy and no TKE/VPC runner Docker build/push
-> separately authorize repo-native run-scoped platform runner Job preflight with unique `medopl-platform-runner-preflight-<runid>` name, `node.tke.cloud.tencent.com/machineset=np-cbk784r8`, and real `PACKAGE_D_RUNNER_IMAGE_REF`; observe/evidence/cleanup only that Job
