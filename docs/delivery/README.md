...
当前 product cursor 是 `opl-webui-runtime-production-slice`，状态是 local/cloud RC only / before production-complete claim。MedOPL 的当前 delivery truth 是：平台给 OPL-Webui 登录用户在当前 workspace 提供托管 runtime、文件空间、计费、审计和释放；MedOPL 不承担 OPL 自身科研能力。最新 product RC closeout 是 `feat/v22-local-product-rc-single-flow` / `4f195820dcb351030cb1064172bfec2062abc279`。当前 trunk `75fab6bfa8e0fa0ce095fdf58605b4c698a5cf41` 已落地 Portal UI/UX production hardening，生产 UI claim 仍依赖 root/frontend dependency audit、浏览器可访问性回归、release owner readiness、observability / production deploy receipt 和 canary proof。新鲜 `npm run cloud:rollout:availability` 显示 `https://portal.medopl.cn/healthz` 和 `/readyz` 返回 MedOPL Go backend JSON 200；这只清退旧 public Go health/ready routing blocker，不等于新的 cloud release candidate、production complete 或完整产品 E2E。provider-key live blocker 的本地 closeout 修正生产 Go config / deploy shape：`PORTAL_OPL_PROVIDER_SECRET_ROOT=/tmp/medopl-runtime/provider-secrets` 和 writable `emptyDir` 已由 Go config test 与 cloud rollout shape test 消费。Release Image run `27969071518` 和 Cloud Rollout run `27969132092` 证明 `b362010` 已进入 production-canary 并通过 deploy receipt；live_test 已越过 provider-key、run_task artifactRef 和 fetch_artifact，随后因 billing summary 缺顶层 `runCount` / `ledgerCount` fail closed。本地 Go API 已补顶层 billing counts 并由 API contract、handler/service tests 消费；Release Image run `27969907029` 已 build `b61a989`，Cloud Rollout run `27969980310` 已 rollout 到 production-canary Pod，但在 deploy receipt / live_test 前因 post-rollout public `healthz` probe 单次检查失败而 fail closed。当前 rollout helper 已把 post-rollout public health probe 改为有界重试，并继续校验 MedOPL Go backend JSON 而不是静态 HTML。Release Image run `27970975355` build `e4306f2`，Cloud Rollout run `27971025234` 已通过 rollout apply 和 deploy receipt，随后 live_test 在 `release_runtime` 因缺顶层 `auditEventId` fail closed；本地 API contract 和 Go ReleaseResult 已补该字段。`verify:cloud-release-candidate` 仍是 evidence-dependent gate；完整产品 E2E 仍必须由授权 `live_test` 和 receipt manifest 证明。
