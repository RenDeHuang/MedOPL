...
当前 product cursor 是 `real-cloud-authorization-boundary`，状态是 authorization-required / before real-cloud readiness。pre-cloud、本地 RC、Package C live canary、Package D production deploy apply 和 Package D in-cluster HTTP reachability 已各自形成 evidence，但它们不等于正式稳定上线。最新 landed repo closeout 是 `recovery/platform-v22-trunk` / `bf8c0f108c80be82cfc00e6729f8f210df23940e`，本轮 closeout pointer 会回填为 `PENDING_GAP08E_COMMIT`。当前 delivery truth 采用 `goal-current.json` 的 Production Launch Goal / Gap Map 作为机器 cursor 派生：目标是 `platform-provisioned / customer-dedicated` 的多租户 SaaS 托管 OPL 工作台，Portal 是用户/管理员入口，平台代管云资源，workspace lifecycle 通过 ResourceBinding / CloudOperation ledger、billing、audit、quota、rollback、cleanup 和 redacted evidence 闭合。Package D 现在是已部署的 TKE platform-service baseline：`pdrun-20260616-004` 完成四服务 deploy/rollout，`psr-20260617-004` 证明四个 ClusterIP Service HTTP `200` 且 smoke Job 清理成功。Production launch Gap 01-08d 已收口为 repo-native contract/local gates、云端 server-side dry-run evidence 或云端 bounded mutation evidence。Gap 08d 真实 apply 已执行：`Secret/medopl-portal-tls` apply 成功，type `Opaque` 且 `qcloud_cert_id` key 存在但未读取 data；`Ingress/portal-frontend` apply 成功，host `portal.medopl.cn`，ingressClass `qcloud`，tls secret `medopl-portal-tls`，backend `portal-frontend:8080`。Ingress status LB hostname 为 `lb-b33auprw-h1bv86yx9nswdtfj.clb.usw-tencentclb.com`，但 `Ready=False` / `BackendError`，已分类为 qcloud Ingress backend 需要 `NodePort` 或 `LoadBalancer` Service，而现有 backend 是 `ClusterIP`；DNS 仍指向旧 `lb-lhj3bgii...`，HTTPS smoke 为 `tls_hostname_mismatch`。Evidence 为 `.runtime/package-d-external-access-strategy/gap08d-qcloud-ingress-real-004/{real-mutation,qcloud-cert-secret,portal-ingress}-redacted.json`，无越界 mutation。当前唯一 gap 是 `production-launch-gap-08e-qcloud-ingress-backend-nodeport-edge-service`：repo-native runner 已支持 `qcloud-edge-nodeport-dry-run` / `qcloud-edge-nodeport-apply`，策略是保留原 `Service/portal-frontend` ClusterIP 不变，新增专用 `Service/portal-frontend-edge` type `NodePort`，selector 与 portal frontend 一致，port/targetPort `8080`，并仅把 qcloud Ingress backend 切到 `portal-frontend-edge:8080`。该 gap 仍不授权 DB password、Portal admin password、Tencent SecretId/SecretKey、TLS private key/cert material、原 ClusterIP Service 修改、DNS 修改、LoadBalancer Service、Package C/tenant resources、build/push、deploy/rollout/rollback、Package C live 或 public access completion claim。
...
后续真实云工作不得继承 slide authority，也不得跳过产品化路线图；Go control-plane MVP takeover、precloud-deployable-rc、local SaaS backend RC 和 local Portal/OPL delivery RC 均只提供本地 RC evidence。下一步仍留在 `real-cloud-authorization-boundary`，但当前唯一 gap 已从 qcloud TLS Secret + Ingress real apply 推进到 qcloud edge NodePort backend strategy。Package D 已 deployed inside TKE 且 in-cluster HTTP reachability passed；external/public user access not yet complete 仍是事实，下一步只允许在单独授权下创建/应用 `Service/portal-frontend-edge` type `NodePort` 与更新 Portal qcloud Ingress backend 为 `portal-frontend-edge:8080`，并只做 get/describe verification、DNS validation 与 HTTPS smoke；不允许修改原 `Service/portal-frontend` ClusterIP、Deployment、DNS、LoadBalancer Service、Package C/tenant resources，不声明公网可用。当前 cursor 不授权新的 Tencent API mutation、deploy、build/push、Package C live、production PostgreSQL write/read 或手写 Package D execution。
