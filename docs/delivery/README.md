...
后续真实云工作不得继承 slide authority，也不得跳过产品化路线图；Go control-plane MVP takeover、precloud-deployable-rc、local SaaS backend RC 和 local Portal/OPL delivery RC 均只提供本地 RC evidence。下一步仍留在 `real-cloud-authorization-boundary`：Package D `production-deploy-apply` run `pdrun-20260616-004` 已让四个服务在 TKE 内 ready，但当前 Service 仍是 ClusterIP-only；readonly service reachability / in-cluster HTTP smoke run `psr-20260617-004` 已通过：四个 Deployment ready `1/1`，四个 ClusterIP Service `8080/http` 均返回 HTTP `200`，smoke Job 已删除并验证 `NotFound`，redaction audit 通过，期间没有 deploy / rollout / rollback / build-push / Tencent mutation / Package C live。当前 Package D 已 deployed inside TKE 且 in-cluster HTTP reachability passed，但 external/public user access not yet exposed。下一步是 Portal external access strategy authorization pack：比较 admin-only port-forward、internal gateway、Ingress、LoadBalancer 和 HTTPS/domain，不默认选择公网暴露。当前 cursor 不授权新的 Tencent mutation、kubectl、deploy、build/push、Ingress/LB 变更或手写 Package D execution。
...
-> next gap is Portal external access strategy authorization pack; compare admin-only port-forward, internal gateway, Ingress, LoadBalancer and HTTPS/domain, with admin-only port-forward or internal gateway recommended as the next non-public strategy step before any public exposure claim
