...
后续真实云工作不得继承 slide authority，也不得跳过产品化路线图；Go control-plane MVP takeover、precloud-deployable-rc、local SaaS backend RC 和 local Portal/OPL delivery RC 均只提供本地 RC evidence。下一步仍留在 `real-cloud-authorization-boundary`，只推进 Package D production deploy dry-run / execution evidence、post-deploy smoke/rollback evidence 和 productionization plan；当前 cursor 不授权新的 Tencent mutation、kubectl、deploy、build/push 或手写 Package D execution。
...
-> Package D four service image publish readiness through a repo-native private build runner lane; do not use the TKE/VPC deploy runner for docker build/push and do not hand-run ad hoc build/push
