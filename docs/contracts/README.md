...
| UI design execution source | [../../DESIGN.md](../../DESIGN.md) | v22 Portal UI 重构的设计执行源：产品气质、信息架构、组件系统、文案规则、视觉规则、Figma 往返流程和后续 slice。它只把合同和 evalset 转成设计执行规则，不替代合同、evalset 或 smoke。 |
...
- [v22-portal-ui-design-quality-audit-boundary.md](./v22-portal-ui-design-quality-audit-boundary.md): UI design quality audit 合同只管边界、评价标准、audit evidence schema 和后续 UI implementation leaf handoff，不替代 UI composition 合同，不冻结具体布局、配色、字体、圆角或组件库；它审计 Portal 是否回答用户买了什么、能不能用、缺什么、下一步点哪里、结果在哪里和费用是否正常。审计证据路径固定为 `.runtime/portal-ui-design-quality/report.json` 且不进 git；`leaf-portal-ui-design-quality-implementation` 已在 `services/portal/frontend/**` 落地 overview 首屏服务摘要、状态驱动下一步、Portal/OPL runtime 职责边界、环境/套餐/算力/存储/释放状态和文件/任务/结果链路，并同步 surface/visual 验证。
- [../../DESIGN.md](../../DESIGN.md): Portal UI 重构设计执行源，用于指导后续 slice 的产品气质、信息架构、组件使用、文案、视觉规则和 Figma 往返流程；它不替代本合同包、不替代 evalset、不替代 smoke，也不授权修改后端、依赖、真实云、deploy、upstream 或 secret 边界。
