...
- RED: 新增 `tests/contract/contract-test-v22-contract-smoke-eval-index-compaction.mjs` 前，classification 已登记新 gate 时 `smoke-classification-gate` 失败，暴露“分类有、文件不存在”的悬空状态。
...
- `node tests/contract/contract-test-v22-smoke-eval-physical-compaction.mjs`
...
Post-push note: `current` verification exposed a narrow detached-trunk runtime gap in `tests/contract/contract-test-v22-goal-state-consistency.mjs`. The intended follow-up is to allow only detached `HEAD == origin/recovery/platform-v22-trunk`; arbitrary detached HEAD remains fail-closed.
