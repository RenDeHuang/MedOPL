...
- current zero-compat gap: Runtime Bridge active code 仍残留 `resourceOrderId` / `resource_order_id` 持久化或发布，以及 `user_owned` / `USER_OWNED_*` runtime alias；这些不改变 current cursor，但必须在 cleanup branch 清退。
