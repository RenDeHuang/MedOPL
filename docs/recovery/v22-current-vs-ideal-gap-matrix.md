...
- current_fact: Portal UI MVP has been migrated to React + Vite + TypeScript + shadcn/Radix + lucide for the ordinary user Portal routes `/overview`, `/resources`, `/workspace`, `/trace`, `/billing`, and `/opl-launch`. `v22-portal-figma-make-ui-implementation-boundary.md` authorizes this Portal-wide frontend stack and records Figma Make file `pjLYKml89XFsf8BMNOJ3CV` plus ZIP artifact as the ordinary user UI implementation source. Figma `AdminConsole.tsx` residue is explicitly copied but unrouted; Admin / Ops remains a same-stack future UI leaf, not current Figma coverage. DESIGN.md translates contracts and Figma Make truth into implementation guidance without replacing contracts, ZIP source-of-truth, smoke, or v22 product truth.
...
- B_absorb_criteria: B confirms Portal frontend implementation is real React/Vite UI work under `services/portal/frontend/**`, dependency changes are limited to Portal frontend stack migration, Figma Make `AdminConsole.tsx` is excluded, no backend/deploy/upstream/secret/live-cloud boundary is crossed, and Portal remains the SaaS control plane for the托管 OPL 科研工作台服务.

- leaf-frontend-product-evalset-gap: historical Vue-era frontend evalset characterization was absorbed before this React/Figma Make implementation leaf. Current executable user Portal truth now lives in the Figma Make ZIP surface gate and `src/app/data/portalAdapters.ts`. Old visual workbench and screenshot regression evidence are not current completion evidence.
