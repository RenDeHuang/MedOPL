...
| CO-06 | user-authorized readonly live | needs-user-authorization | no live report yet; historical readonly live runner has exited active repo executable surface | user | decide whether to authorize official SDK readonly secret allowlist and readonly API call under a future-authorized boundary | `smoke-test-v22-tencent-readonly-inventory-bridge-local-gate.mjs`; check-config output | must explicitly authorize secret allowlist, region/API scope, real cloud call, report location |
