...
- 租户不开通 runtime 时，可以有账号、充值、绑定 API key、查看 Portal 状态，但不能跑托管 runtime 任务。
...
商业目标之一是销售 token/API 使用额度。用户绑定 API key 时，raw API key 只能进入后端密钥边界；前端最多保留一次性输入态、`providerKeyRef` 和 bound status。raw API key、bearer token、launchToken、runtimeToken 不能写入 sessionStorage、localStorage、global JS state、log、evidence 或 git。
