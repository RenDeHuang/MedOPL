...
- next_local_service_code_leaf: `leaf-portal-postgres-redis-local-production-data-closure` is indexed but not implemented. Its eval shell is `scripts/smoke-test-v22-portal-storage-mode-local-closure.mjs`; target truth is PostgreSQL canonical data, Redis session/cache/queue/lock only, and `PORTAL_STORAGE_MODE=postgres_redis` fail-closed behavior without JSON fallback.
