...
| active | `services/portal`、`services/opl-web-gateway`、`services/opl-runtime-bridge`、`docs/product.md`、`docs/architecture.md`、`docs/specs/v22-*`、`docs/recovery/*`、`tests/**/*.mjs` | 可以承载 v22 新产品语义、契约、文档收敛和本地 smoke |
...
- local_service_code_fact: `feat/v22-portal-opl-file-run-artifact-closure` was B-reviewed, ff-only absorbed and pushed at `8797ffc6f3ba3747cfac55554012b648fcbfb5c9`. It wires existing Portal React/Figma UI actions to `createOplFileRef`, `startOplRun` and `fetchOplArtifact`, and keeps the canonical local closure on `/portal/api/opl/*` plus Runtime Bridge/Gateway projection gates. This is local Portal-OPL closure only: no true cloud, no deploy, no secret read, no upstream modification, and no Figma visual/layout/information-architecture change.
- next_local_service_code_leaf: `leaf-portal-postgres-redis-local-production-data-closure` is indexed but not implemented. Its eval shell is `tests/regression/portal/regression-test-v22-portal-storage-mode-local-closure.mjs`; target truth is PostgreSQL canonical data, Redis session/cache/queue/lock only, and `PORTAL_STORAGE_MODE=postgres_redis` fail-closed behavior without JSON fallback.
...
- Leaf 7 productionization eval shell: `tests/regression/opl/regression-test-v22-opl-productionization-eval-shell.mjs` is a local repo-tracked gate for the next OPL production implementation branch.
...
- v22 program board: [spec:v22-program-board](./v22-program-board.md)
- v22 program status table: [spec:v22-program-status-table](./v22-program-status-table.md)
