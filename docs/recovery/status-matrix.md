...
| Portal Web | `services/portal/frontend/src/app/routes.tsx`、`services/portal/frontend/src/app/App.tsx`、`services/portal/frontend/src/app/components/Layout.tsx`、`services/portal/frontend/src/app/pages/Overview.tsx`、`services/portal/frontend/src/app/pages/RuntimeEnvironment.tsx`、`services/portal/frontend/src/app/pages/Workspace.tsx`、`services/portal/frontend/src/app/pages/OPLEntry.tsx`、`services/portal/frontend/src/app/pages/BillingAudit.tsx`、`services/portal/frontend/src/app/pages/TasksResults.tsx`、`services/portal/frontend/src/app/data/portalAdapters.ts`、`services/portal/frontend/src/api/portal/*.ts`、`services/portal/src/routes/portal-api.routes.mjs`、`services/portal/src/routes/opl.routes.mjs`、`services/portal/src/routes/lab-package.routes.mjs`、`services/portal/src/routes/workspace.routes.mjs`、`services/portal/src/routes/workspace-storage.routes.mjs` | current user Portal UI 是 Figma Make ZIP source-of-truth + React + Vite + TypeScript + react-router + shadcn/Radix + lucide；Admin / Ops UI 后续同栈单独 leaf | legacy redirect route 已删除；旧 `/portal/app/*`、旧 `/portal/admin/*` redirect、`/packages`、`/advanced/servers`、`/admin/*` 和 `/portal/tasks/*` 入口不得恢复为当前 frontend route | 旧 Vue/Pinia SPA、visual workbench、screenshot baseline、上一轮非 ZIP 1:1 React shell 和旧 resource-order 页面/路由叙事不作为 Portal Web 主线 |
...
| Admin / Ops | `services/portal/src/routes/admin-api.routes.mjs`、`admin-user.routes.mjs`、`admin-ops.routes.mjs`、`services/portal/src/app/portal-admin-overview-payloads.mjs`、`portal-admin-api-payloads.mjs`、`portal-admin-portrait-payloads.mjs` | admin payload 和后续前端文案中旧 `user_owned`、OpenCost、Langfuse、runner/provisioner 表述要迁到平台托管资源池和 metadata boundary；当前 Figma Make `AdminConsole.tsx` 是残留，不是可吸收 Admin UI | 后续 cleanup 删除把 Rancher/OpenCost/KubeSphere/runner 暴露成用户产品面的入口；Admin / Ops 前端须后续同 React 技术栈单独 leaf | 旧 ops 工具和旧栈画像只作运维参考，不进入用户主线；旧 Vue admin views 已从当前 Portal frontend surface 清退 |
