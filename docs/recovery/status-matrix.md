...
| archive/reference | `docs/plan/*`、`docs/reports/*`、`docs/releases/*`、`docs/logs/*`、`scripts/smoke-test-v19-*`、`scripts/smoke-test-v20*`、`OPL-v20-商业化产品套餐开发方案.md` | 只读历史证据和迁移参考，不作为新实现入口或默认验证入口；旧 `scripts/live-test-*` 已由授权物理删除 slice 处理 |
...
| Scripts / Contracts | `docs/contracts/v22-*.md`、`docs/product.md`、`docs/architecture.md`、`docs/recovery/*`、`scripts/smoke-test-v22-*` | `scripts/smoke-test-v20*`、`scripts/smoke-test-v21-*` 中仍有价值的合同要迁成 v22 命名和 truth；`README.md` 和旧说明文档后续继续清理残留 v21 口径 | live-test 作为普通验证入口的使用说明已由授权物理删除 slice 清理 | `scripts/smoke-test-v19-*`、`scripts/smoke-test-v20*`、`scripts/check-v21-user-owned-runtime-boundaries.mjs`、`scripts/smoke-test-v21-*user-owned*.mjs`、旧 v19/v20 合同只作历史参考；旧 `scripts/live-test-*` 文件已物理删除 |
