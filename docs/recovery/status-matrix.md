...
| Billing / Usage / Freeze | `services/portal/src/domain/wallet-ledger.mjs`、`lab-billing-policy.mjs`、`services/portal/src/app/portal-page-billing-payloads.mjs`、`services/portal/src/integrations/billing-client.mjs`、`services/portal/frontend/src/api/portal/billing.ts`、`services/portal/frontend/src/views/billing/BillingView.vue`、`services/portal/frontend/src/views/admin/AdminBillingOpsView.vue` | v22 账单叙事是预扣费、冻结金额、7 天保护和释放停止扣费；zero-compat 下 `adapters/billing-aggregator/**` 不能以 adapter 形态作为 active repo 默认上下文保留 | Slice K 必须删除 residual billing adapter compatibility surface；若仍需能力，先迁入 Portal billing domain 或 v22 service boundary | `scripts/start-opencost-*`、`scripts/install-opencost-local.ps1`、`scripts/smoke-test-billing-opencost.mjs`、旧 non-v22 billing smoke、残余 non-v22 billing smoke 和 `scripts/start-billing-live.mjs` 已删除 |
