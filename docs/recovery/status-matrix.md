...
| Scripts / Contracts | `docs/contracts/v22-*.md`、`docs/product.md`、`docs/architecture.md`、`docs/recovery/*`、`scripts/smoke-test-v22-*` | 有价值的旧合同必须迁成 v22 命名和 truth 后保留；旧脚本本体不保留 | live-test 作为普通验证入口的使用说明已清理 | `scripts/smoke-test-v19-*`、`scripts/smoke-test-v20*`、`scripts/smoke-test-v21-*`、旧 check/daily/live-prepare/resource-provisioner/OpenCost 脚本已在 Slice C 删除；旧 v13、portal resource-order/provisioner、runner fixture、v19/v20 helper lib 已在 Slice D 删除；旧 non-v22 billing/portal smoke 已在 Slice E 删除 |
