...
| Deploy / Provisioning | 无 strict monolith default executable surface | zero-compat 下 `deploy/local/dockerfiles/portal.Dockerfile`、`opl-web-gateway.Dockerfile`、`opl-runtime-bridge.Dockerfile` 不作为 active repo 默认上下文；未来真实部署只保留合同边界，需重新授权和重新建 v22 deploy surface | Slice L 必须删除 residual deploy compatibility assets；不得在本 cleanup 分支执行 build/push/kubectl/deploy | `deploy/tke-package/**`、旧 runner/provisioner Dockerfile 已删除；residual local Dockerfiles 待 Slice L 裁定删除 |
| Scripts / Contracts | `docs/contracts/v22-*.md`、`docs/product.md`、`docs/architecture.md`、`docs/recovery/*`、`scripts/smoke-test-v22-*`、`scripts/v22-verify.mjs` | 有价值的旧合同必须迁成 v22 命名和 truth 后保留；旧脚本本体不保留；MVP suite 可以通过 `scripts/v22-verify.mjs` 统一 runner 执行 branch-appropriate bundle | live-test 作为普通验证入口的使用说明已清理 | `scripts/smoke-test-v19-*`、`scripts/smoke-test-v20*`、`scripts/smoke-test-v21-*`、旧 check/daily/live-prepare/resource-provisioner/OpenCost 脚本已在 Slice C 删除；旧 v13、portal resource-order/provisioner、runner fixture、v19/v20 helper lib 已在 Slice D 删除；旧 non-v22 billing/portal smoke 已在 Slice E 删除；残余 non-v22 Portal/Billing smoke 和本地 start/install helper remnants 已在 residual Phase G 删除 |
