...
- local_service_code_fact: `feat/v22-portal-opl-file-run-artifact-closure` wires existing Portal React/Figma UI actions to `createOplFileRef`, `startOplRun` and `fetchOplArtifact`, and keeps the canonical local closure on `/portal/api/opl/*` plus Runtime Bridge/Gateway projection gates. This is local Portal-OPL closure only: no true cloud, no deploy, no secret read, no upstream modification, and no Figma visual/layout/information-architecture change.
