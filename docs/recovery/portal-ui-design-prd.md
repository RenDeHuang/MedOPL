...
- 管理员能在独立管理台查看客户账户、服务状态、任务记录、账单处理和审计记录，但普通用户 surface 不串台。
...
管理员负责客户账户、资源管理、任务记录、账单管理、审计记录、站点设置和服务状态。管理员页面可以展示更专业的运营和排障信息，但必须与普通用户主线隔离。
...
- 管理台页面：管理总览、客户账户、站点设置、服务状态、账单处理、任务记录、审计记录，以及现有 portrait / ops 类页面。
...
5. 管理与排障：客户账户、任务记录、账单处理、服务状态、审计记录。
...
- 提供客户账户、任务记录、账单处理、审计记录和站点设置入口。
- 不把管理总览暴露给普通用户。

### 8.10 客户账户 `/admin/users`
