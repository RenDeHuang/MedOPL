...
- `scripts/v22-local-services.mjs` owns the repo-native local service plan and local URL health probe for Portal frontend, Go backend, OPL Web Gateway, Runtime Bridge and external clean OPL WebUI. It does not start services, read secrets, deploy, kubectl, build/push or call real cloud.
