...
| current blocker | G3 已证明 real cloud business closure；当前阻塞转为 G4 deployment proof：确认当前 trunk/image 已部署或执行 release image + rollout，验证 `portal.medopl.cn` HTTPS / redirect / healthz / readyz、OPL-Webui consumer entry、G3 evidence artifact restore、runtime/storage/billing/audit/release/stop-billing receipts 可追溯、rollback owner/command、monitoring/alerting owner，以及 cost ceiling 由 account balance / plan / quota 控制。`MEDOPL_PRODUCTION_*` 只属于 production launch operations safety，不是 runtime gate、充值、套餐、余额、quota 或 billing truth；legacy selected-user operations envs 不再是 G4 blocker。 |
