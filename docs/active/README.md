...
| current blocker | Package D is deployed inside TKE and in-cluster HTTP reachability has passed: the runner image and four service images are published, `portal-frontend`, `medopl-go-backend`, `opl-web-gateway` and `opl-runtime-bridge` Deployments are ready `1/1`, all four ClusterIP Services return HTTP `200`, and redacted evidence is recorded at `.runtime/package-d-production-deploy/pdrun-20260616-004/{deploy,smoke,rollback}-redacted.json` plus `.runtime/package-d-service-reachability/psr-20260617-004/{readonly-service-reachability,smoke-job-manifest}-redacted.json`. Production launch Gap 01 through Gap 08b are repo-native and local-gated, and Gap 08c qcloud TLS Secret + Ingress server-side dry-run passed in cloud at HEAD `abbc82f3bbd39d2c659c514476153f36ccb63d9f` with evidence `.runtime/package-d-external-access-strategy/gap08c-qcloud-ingress-dry-run-001/dry-run-redacted.json`; both `medopl-portal-tls` and the Portal Ingress remained `NotFound`, no real mutation occurred and redaction audit passed. The first Gap 08d real mutation attempt at HEAD `259679e1f54c39a8a0b8039a927e1b735e1e63a3` failed closed because the repo lacked a real mutation runner, the env lacked `PORTAL_HOST_DOMAIN` / `INGRESS_CLASS` / `TLS_SECRET_NAME` / `TENCENT_SSL_CERT_ID` / `EXTERNAL_SMOKE_URL`, and no TLS material path was authorized; evidence is `.runtime/package-d-external-access-strategy/gap08d-qcloud-ingress-real-001/real-mutation-redacted.json`, and no resource was created. Gap 08d-prep is now repo-native: `tests/support/cloud-prework/package-d-external-access-runner.js` supports `qcloud-ingress-dry-run` and `qcloud-ingress-apply`, the local gate proves env allowlist, redaction, command allowlist, and qcloud Secret shape `Opaque` + `stringData.qcloud_cert_id`. `TENCENT_SSL_CERT_ID` remains an authorized env value only and its raw value must not enter docs, logs or evidence. External/public user access remains not exposed. The next unique gap is `production-launch-gap-08d-qcloud-opaque-cert-id-secret-ingress-real-apply`: cloud execution must first place the five external access env keys into an authorized env file, set `RUN_TENCENT_DEPLOY_EXECUTION=external-access`, and then run the repo-native apply runner limited to `Secret/medopl-portal-tls` and the qcloud Portal Ingress for `portal.medopl.cn -> portal-frontend:8080`, followed by get/describe verification, DNS post-apply validation and HTTPS smoke; it still must not claim public access complete before post-apply evidence. |
