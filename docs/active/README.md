...
当前 trunk 已完成合同级闭环、本地 deterministic eval、本地 smoke/local proof、Portal Workspace 文件动作闭环、Portal-OPL file/run/artifact 本地闭环、slide-01 data truth 本地闭环、slide-02 Portal API real data wiring 本地闭环，以及 slide-03 account/wallet/billing 本地闭环。当前不是真实云生产闭环，不是 workspace/files 全语义完成态，也不是 admin 全业务闭环完成态。

当前 product cursor 是 `leaf-workspace-files-closure`。当前 active baton 是 `slide-04-workspace-files`：workspace、file space、fileRef、file list 和 7 天保护语义必须 owner-scoped 且可本地查询，不得把 localPath、objectKey、signedUrl 或无归属文件状态暴露为前端产品真相。slide-01 已让 PostgreSQL 成为 local production data canonical truth；slide-02 已让 RuntimeEnvironment 可见套餐、订阅和权益状态来自 typed Portal lab API client；slide-03 已让账务 ledger、冻结净额、pending usage、T+1 settlement 和 BillingAudit 账户归属显示进入 owner-scoped 本地闭环。

最近已通过 landing gate 的产品闭环是 `feat/v22-slide-03-account-wallet-billing`，landed commit 为 `525546b7fb5c4483edeb3f21db4cd7a675996540`。该分支统一了 owner-scoped wallet ledger、frozen amount、pending usage、T+1 settlement 和 BillingAudit UI-safe account scope display；当前 closeout 已把 active baton 推进到 slide-04。
...
- Workspace、file space、fileRef、file list 和 7 天保护语义仍需 owner-scoped 本地闭环。
...
| Product engineering loop | pre-cloud product slides must run as an active baton, not permanent planning prose | `tests/fixtures/v22/goal-current.json` `product_engineering_loop` `precloud-product-slides-closure` + manifest `product-engineering-loop` suite | slide-01 到 slide-03 已 landed；当前 open baton 是 slide-04，open 明细只在 current machine fixture 临时存在 | run slide-04; then continue slide-05..slide-09 in order | 9 个产品 slide 全部闭合后 collapse to history summary and next cursor，active truth 不保 slide 明细、per-slide docs、compat layer 或 shadow archive | `node tests/contract/contract-test-v22-product-engineering-loop-index.mjs` |

## Current Development Lines

### current-stage-current-cursor

Current evidence: latest landed product closeout is `525546b7fb5c4483edeb3f21db4cd7a675996540`; current machine cursor is `leaf-workspace-files-closure`.

Gap: slide-01 data truth, slide-02 Portal API real data wiring and slide-03 account/wallet/billing closure are complete, but slide-04 workspace/files closure is still gated and not implemented.

Next action: implement slide-04 so workspace, file space, fileRef, file list and protection state are owner-scoped and locally queryable, while retaining slide-01/slide-02/slide-03 regressions as guards.

Done when: readers can distinguish landed slide-01/slide-02/slide-03 facts from unfinished slide-04 workspace/files work.
...
Current evidence: MedOPL is the SaaS control plane and managed delivery platform for clean One Person Lab; Portal owns account, workspace, balance, files, billing and trace surfaces while OPL owns scientific execution inside the workbench. The active baton is `slide-04-workspace-files`.

Gap: product slides must keep the user loop visible instead of collapsing the product back into architecture or cloud-console language. slide-01 through slide-03 have landed; slide-04 still needs to close owner-scoped workspace, file space, fileRef, file list and protection semantics.
