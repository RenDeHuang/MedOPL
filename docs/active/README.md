...
| current blocker | Package D is deployed inside TKE and in-cluster HTTP reachability has passed: the runner image and four service images are published, `portal-frontend`, `medopl-go-backend`, `opl-web-gateway` and `opl-runtime-bridge` Deployments are ready `1/1`, all four ClusterIP Services return HTTP `200`, and redacted evidence is recorded at `.runtime/package-d-production-deploy/pdrun-20260616-004/{deploy,smoke,rollback}-redacted.json` plus `.runtime/package-d-service-reachability/psr-20260617-004/{readonly-service-reachability,smoke-job-manifest}-redacted.json`. Production launch Gap 01 through Gap 08e qcloud edge NodePort apply has reached the cloud boundary. In `gap08e-qcloud-edge-nodeport-apply-001`, `Service/portal-frontend-edge` was applied successfully as type `NodePort` on port `8080`, the original `Service/portal-frontend` remained `ClusterIP`, and `Ingress/portal-frontend` now targets `portal.medopl.cn`, `ingressClass=qcloud`, `tls secret=medopl-portal-tls` and backend `portal-frontend-edge:8080`. The Ingress is `Ready=True` with LB hostname `lb-b33auprw-h1bv86yx9nswdtfj.clb.usw-tencentclb.com`; DNS `portal.medopl.cn` still points to old LB `lb-lhj3bgii-ms5ocrjz6hdaki2l.clb.usw-tencentclb.com`, and HTTPS smoke did not produce valid evidence because the cloud runner curl lacked `--fail-with-body` compatibility. Evidence is `.runtime/package-d-external-access-strategy/gap08e-qcloud-edge-nodeport-apply-001/{real-mutation,portal-frontend-edge-service,portal-ingress}-redacted.json`, no out-of-bound mutation occurred, and external/public user access remains not complete. The current unique gap is `production-launch-gap-08f-dns-validation-https-smoke`: align DNS through separate authorization, then run repo-native DNS validation and external HTTPS smoke against only `https://portal.medopl.cn/` with curl compatibility fallback and redacted structured evidence. |
