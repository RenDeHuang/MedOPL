...
| latest repo closeout | `recovery/platform-v22-trunk` / `f788f0019c0f2661617ffebe2d27f3fcfd10d709` |
...
- `package-d-vpc-deploy-runner-plan`: Commercial stabilization no longer treats the current WSL session as the Package D execution environment. The target is a dedicated VPC deploy runner named `medopl-v22-deploy-runner` in `na-siliconvalley` / `medopl-vpc` / `medopl-private-a`, with `git`, `node/npm`, `go`, `docker` and `kubectl`; it will hold only local secret files at `/home/dev/.secrets/medopl/v22/package-d-deploy.env`, `/home/dev/.secrets/medopl/v22/portal-runtime.env` and `/home/dev/.secrets/medopl/v22/kubeconfig-package-d-deploy`. PostgreSQL and TKE API stay private, SSH is limited to operator IP or cloud assistant / console access, and runner state must not enter git. This is an implementation plan only: no CVM, Tencent mutation, kubectl, deploy, build/push or Package D execution has run.
