...
| cloud proof meaning | `cloud release candidate` evidence from authorized Cloud Rollout and redacted receipt manifest; still not production complete |
...
| latest repo closeout | `recovery/platform-v22-trunk` / `3eac187ab94009b4f6b477b5eaacd522c4bd2f07` |
Current summary: 当前主线是 MedOPL 作为产品控制面，为 OPL-Webui 的 `runtime_required` 任务提供 runtime gate、runtime/storage 开通、node pool projection、billing/freeze/audit/release 和 storage destroy intent。OPL-Webui 仍然负责登录后的 ordinary chat 和用户交互入口；MedOPL 不负责 OPL 科研能力质量。最新 local product RC 是 `feat/v22-local-product-rc-single-flow` / `4f195820dcb351030cb1064172bfec2062abc279`，已把 `runtime gate -> upload file -> fileRef -> run -> artifact -> billing/audit -> release stop billing -> explicit storage destroy` 收口进 golden smoke。The latest repo/gate closeout is recovery/platform-v22-trunk at 3eac187ab94009b4f6b477b5eaacd522c4bd2f07. Phase A PostgreSQL Business Persistence RC and Phase E billing idempotency hardening are now source-landed and live-rollout verified for the current authorized canary path: production Go backend fails closed without `DATABASE_URL`, selects a Postgres control-plane store in production, keeps local/test on memory, persists account/provider/runtime/storage/file/run/artifact/audit/release metadata through the store, and uses typed `business_accounts` / `credit_events` tables for idempotent and atomic credit balance updates, and stores billing events by non-empty source idempotency key. 当前 trunk 保持 Portal UI/UX production gates：root/frontend 生产依赖 audit、浏览器可访问性回归、release owner readiness、observability / production deploy receipt、S-level UI polish gate、production-complete owner gate 和 canary proof 都必须在生产 claim 前满足。GitHub v22 verify run `28057359860` passed on `70db4e5`; Release Image run `28057445074` built image tag `70db4e5`; Cloud Rollout run `28057493445` deployed image tag `70db4e5` and passed production DB auth/secret sync, runtime/storage/billing-audit owner receipt lanes, kubectl rollout, deploy receipt, Portal public entry, HTTP-to-HTTPS redirect, Go health/ready availability, OPL-Webui consumer canary, production receipt manifest, `verify:cloud-release-candidate`, and `verify:production-complete-candidate`. The redacted manifest has seven owner receipts, 12 lifecycle sections, eight production-complete criteria including `business_db_persistence_receipt`, `claim=production_complete`, `state=complete`, sha256 `6c095a4ceddef604c53986b2cf0fcc0e2fb551b0a81ee1d86b06328463affd23`, and live DB proof hash `sha256:e61986f084e8867d9a87d4037a531cbd89263cd67600109890139aabe13ef9d8`. This can claim cloud release candidate and production-complete candidate evidence for the current authorized canary path only. It still does not claim final production complete release decision, multi-region/SLA, enterprise compliance, ongoing authorization, broader unobserved tenants/resources, or long-running stability. Raw workflow evidence, secrets, kubeconfig, cloud payloads, runtime artifacts, uploaded files and transcripts stay outside git.
...
- `opl-webui-runtime-production-slice`: active cursor. It now has cloud release candidate evidence for the current authorized canary path, while production complete remains fail-closed.
...
- `cloud-future-authorized`: keeps Package C dry-run plan, TKE bootstrap preflight local gates and the single `cloud:goal -- --operation <operation_class>` authorization-pack runner plumbing. Goal B may execute local no-cloud plans; Goal A/C/D/E have accepted owner receipt pointers; Goal F has current remote cloud RC / production-complete candidate evidence through Release Image run `28057445074`, Cloud Rollout run `28057493445` and the redacted receipt manifest, now including Phase E billing idempotency hardening. Final production complete release decision remains fail-closed.
