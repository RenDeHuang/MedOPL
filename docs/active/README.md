...
| latest repo closeout | `chore/v22-closeout-32b1502-rollout-dns-blocker` / `1c97e75837a94360fc410954fdfc04331116375e` |
Current summary: 当前主线是 MedOPL 作为产品控制面，为 OPL-Webui 的 `runtime_required` 任务提供 runtime gate、runtime/storage 开通、node pool projection、billing/freeze/audit/release 和 storage destroy intent。OPL-Webui 仍然负责登录后的 ordinary chat 和用户交互入口；MedOPL 不负责 OPL 科研能力质量。最新 local product RC 是 `feat/v22-local-product-rc-single-flow` / `4f195820dcb351030cb1064172bfec2062abc279`，已把 `runtime gate -> upload file -> fileRef -> run -> artifact -> billing/audit -> release stop billing -> explicit storage destroy` 收口进 golden smoke。当前 trunk `32b1502cb128cca1979afb07e25dac075b6c7417` 已通过 GitHub `v22 verify` run `27913474525`，Release Image run `27913590845` 成功并推送 image tag `32b1502`；Cloud Rollout run `27913603345` 已执行 runtime/storage/billing/kubectl 前置 receipt lanes 并把 `medopl-control-plane` rollout 到 image `32b1502`，但在 `Rollout apply` 的 Go backend JSON health gate fail closed。诊断显示 Kubernetes Ingress `medopl` 的地址仍是 `lb-7trlq374-v40s3uxfnpb1dazj.clb.usw-tencentclb.com`，Service endpoint 是 `10.66.0.133:8080`，但 GitHub runner DNS 仍把 `portal.medopl.cn` 解析到旧静态 nginx CLB `lb-pwv9zgky-yqsc8g20f1o6p3ir.clb.usw-tencentclb.com` / `43.159.159.90`；`/`、`/healthz`、`/readyz` 和 `/api/opl/runtime-gate` 因此仍返回 `text/html`，不是 MedOPL Go backend JSON。因此完整 Goal G E2E canary 仍未通过，也没有新的 cloud release candidate receipt manifest。当前 gate 已加固：rollout apply、availability probe 和 `live_test` 都必须 fail closed，除非 MedOPL public API 完成 `healthz/readyz -> provider-key -> open runtime -> runtime-gate -> upload -> run -> artifact -> billing -> release -> storage destroy` 的公网 API flow。raw workflow evidence、secret、kubeconfig、cloud payload、runtime artifact、用户上传文件和 agent transcript 仍不得进入 git.
...
- `cloud-future-authorized`: keeps Package C dry-run plan, TKE bootstrap preflight local gates and the single `cloud:goal -- --operation <operation_class>` authorization-pack runner plumbing. Goal B may execute local no-cloud plans; Goal A/C/D/E have accepted owner receipt pointers; Goal F has prior remote cloud RC evidence through Release Image run `27911002260` and Cloud Rollout run `27911028428`; the latest hardened rollout on run `27913603345` correctly fails closed before any cloud RC claim because `portal.medopl.cn` DNS still points at the static nginx CLB instead of the active qcloud Ingress CLB for `medopl-control-plane`.
