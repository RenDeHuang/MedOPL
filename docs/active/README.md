...
| current blocker | Package D is deployed inside TKE and in-cluster HTTP reachability has passed: the runner image and four service images are published, `portal-frontend`, `medopl-go-backend`, `opl-web-gateway` and `opl-runtime-bridge` Deployments are ready `1/1`, all four ClusterIP Services return HTTP `200`, and redacted evidence is recorded at `.runtime/package-d-production-deploy/pdrun-20260616-004/{deploy,smoke,rollback}-redacted.json` plus `.runtime/package-d-service-reachability/psr-20260617-004/{readonly-service-reachability,smoke-job-manifest}-redacted.json`. Production launch Gap 01 is repo-native and local-gated through `tests/support/cloud-prework/production-launch-bootstrap-runner.js`. Production launch Gap 02 is now repo-native and local-gated through `tests/support/cloud-prework/production-launch-operation-runner.js`: it proves unauthorized fail-closed behavior, Portal action shape, Go backend operation request shape, Package C runner invocation boundary, ResourceBinding `requested` / `creating` / `ready` state contract, providerKeyRef-only boundary, idempotency, redacted evidence and external-access block without reading secrets/kubeconfig or executing cloud/deploy/Package C live work; evidence path is `.runtime/production-launch-operation/<runid>/operation-contract-redacted.json`. This is still contract-only production operation readiness, not formal launch. External/public user access remains `blocked` until the multi-tenant minimum closes. The next unique gap is `production-launch-gap-03-resourcebinding-postgresql-ledger-live-write-read-contract`: ResourceBinding / CloudOperation PostgreSQL ledger live write/read contract before billing/audit/quota closure, workspace lifecycle, canary/rollback or external access. |
