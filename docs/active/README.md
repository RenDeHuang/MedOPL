...
| Field | Value |
| --- | --- |
| current phase | `G4 production-canary deployment proof upload_file writeback fix` |
| current cursor | `goal-g4-upload-file-billing-event-writeback-fix` |
| current blocker | G4 production_launch 已部署 `1e33b38` diagnostic image and reached OPL-Webui consumer canary, but `upload_file` fails at billing event writeback. Redacted diagnostic receipt shows `errorCategory=billing_event_write_failed`, `dbOperationStage=save_billing_event`, `handlerStage=upload_file_handler`, launch lookup succeeded, runtime/storage ready, file save succeeded and audit save succeeded. This must be fixed without changing business truth, bypassing billing/audit, fake-successing upload_file or degrading billing write failure to warning. |
| next owner | `MedOPL Platform` owns upload_file billing event writeback; `MedOPL Operations` owns later G4 deployment proof after consumer canary passes |
...
| latest repo closeout | `goal-g4-upload-file-diagnostic-receipt` / `1e33b3829194d566877f8b46821616b189e8deed` |
Current summary: 当前主线是 MedOPL 作为产品控制面，为 OPL-Webui 的 `runtime_required` 任务提供 runtime gate、runtime/storage 开通、node pool projection、billing/freeze/audit/release 和 storage destroy intent。OPL-Webui 仍然负责登录后的 ordinary chat 和用户交互入口；MedOPL 不负责 OPL 科研能力质量。最新 local product RC 是 `feat/v22-local-product-rc-single-flow` / `4f195820dcb351030cb1064172bfec2062abc279`，已把 `runtime gate -> upload file -> fileRef -> run -> artifact -> billing/audit -> release stop billing -> explicit storage destroy` 收口进 golden smoke。The latest repo/gate closeout is goal-g4-upload-file-diagnostic-receipt at 1e33b3829194d566877f8b46821616b189e8deed. Cloud SaaS productization has landed G1 account/recharge/plan productization, G2 OPL-Webui runtime_required consumer refs projection, G3 real cloud business closure, legacy operations-safety retirement and the upload_file redacted diagnostic receipt. The next cursor is `goal-g4-upload-file-billing-event-writeback-fix`: fix the production_launch OPL-Webui consumer canary failure at `upload_file -> save_billing_event` without changing business truth or bypassing billing/audit. Cost ceiling for business claims comes from account balance, plan hold amount and quota, not production launch safety env. Cloud Rollout workflow inputs generate the redacted production launch approval receipt; `MEDOPL_PRODUCTION_*` GitHub vars/secrets are not required, and production launch approval is not business admission, recharge, quota, cost ceiling or billing truth. The existing diagnostic rollout can claim only redacted upload_file diagnostic evidence; consumer canary pass, cloud deployment proof for the commercial closure, external PSP settlement, full OPL-Webui resume implementation, unscoped production complete, multi-region/SLA, enterprise compliance, ongoing authorization and unobserved tenants/resources remain unclaimed. Raw workflow evidence、secrets、kubeconfig、cloud payloads、runtime artifacts、uploaded files 和 transcripts 继续留在 `.runtime` 或外部 evidence store，不进入 git。
...
- `goal-g4-upload-file-billing-event-writeback-fix`: active cursor. It must fix the production_launch OPL-Webui consumer canary failure at `upload_file -> save_billing_event` while preserving billing/audit fail-closed behavior and redacted diagnostic receipts.
