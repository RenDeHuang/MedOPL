...
| Field | Value |
| --- | --- |
| current phase | `G4 production-canary deployment proof` |
| current cursor | `goal-g4-production-canary-deployment-proof` |
| current blocker | G3 已证明 real cloud business closure；当前阻塞转为 G4 deployment proof：确认当前 trunk/image 已部署或执行 release image + rollout，验证 `portal.medopl.cn` HTTPS / redirect / healthz / readyz、OPL-Webui consumer entry、G3 evidence artifact restore、runtime/storage/billing/audit/release/stop-billing receipts 可追溯、rollback owner/command、monitoring/alerting owner，以及 cost ceiling 由 account balance / plan / quota 控制。Cloud Rollout `workflow_dispatch` + GitHub `production` environment approval + `confirm_production_launch=deploy-current` is the one-person production launch approval gate; `MEDOPL_PRODUCTION_*` GitHub vars/secrets are not required and production safety is not runtime gate, recharge, plan, balance, quota or billing truth. |
| next owner | `MedOPL Operations` owns G4 deployment proof and production launch operations safety; `MedOPL Platform` owns account-approved commercial admission and billing truth |
...
| latest repo closeout | `feat/goal-g4-one-person-production-launch-approval` / `39ccf31a85df0124d12a881df809898084ab060e` |
Current summary: 当前主线是 MedOPL 作为产品控制面，为 OPL-Webui 的 `runtime_required` 任务提供 runtime gate、runtime/storage 开通、node pool projection、billing/freeze/audit/release 和 storage destroy intent。OPL-Webui 仍然负责登录后的 ordinary chat 和用户交互入口；MedOPL 不负责 OPL 科研能力质量。最新 local product RC 是 `feat/v22-local-product-rc-single-flow` / `4f195820dcb351030cb1064172bfec2062abc279`，已把 `runtime gate -> upload file -> fileRef -> run -> artifact -> billing/audit -> release stop billing -> explicit storage destroy` 收口进 golden smoke。The latest repo/gate closeout is feat/goal-g4-one-person-production-launch-approval at 39ccf31a85df0124d12a881df809898084ab060e. Cloud SaaS productization has landed G1 account/recharge/plan productization, G2 OPL-Webui runtime_required consumer refs projection, G3 real cloud business closure and legacy operations-safety retirement. The next cursor is `goal-g4-production-canary-deployment-proof`: prove deployment image/rollout/public availability/G3 evidence restore/rollback/monitoring readiness without treating production launch safety as business admission. Cost ceiling for business claims comes from account balance, plan hold amount and quota, not production launch safety env. Cloud Rollout workflow inputs generate the redacted production launch approval receipt; `MEDOPL_PRODUCTION_*` GitHub vars/secrets are not required, and production launch approval is not business admission, recharge, quota, cost ceiling or billing truth. The existing authorized Cloud Rollout manifest can still claim only its scoped cloud release candidate / production-complete candidate evidence; cloud deployment proof for the commercial closure, external PSP settlement, full OPL-Webui resume implementation, unscoped production complete, multi-region/SLA, enterprise compliance, ongoing authorization and unobserved tenants/resources remain unclaimed. Raw workflow evidence、secrets、kubeconfig、cloud payloads、runtime artifacts、uploaded files 和 transcripts 继续留在 `.runtime` 或外部 evidence store，不进入 git。
...
- `goal-g4-production-canary-deployment-proof`: active cursor. It must prove deployment image/rollout/public availability/G3 evidence restore/rollback/monitoring readiness without expanding into full production complete or treating production launch safety as commercial admission.
