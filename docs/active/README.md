...
当前 trunk 已完成合同级闭环、本地 deterministic eval、本地 smoke/local proof、Portal Workspace 文件动作闭环、Portal-OPL file/run/artifact 本地闭环、slide-01 data truth 本地闭环，以及 slide-02 Portal API real data wiring 本地闭环。当前不是真实云生产闭环，不是 account/wallet/billing 全语义完成态，也不是 admin 全业务闭环完成态。

当前 product cursor 是 `leaf-account-wallet-billing-closure`。当前 active baton 是 `slide-03-account-wallet-billing`：account、wallet、balance、frozen amount、ledger 和 audit 语义必须 owner-scoped 且可本地查询，不得把 OpenCost/Langfuse 或无归属 amount state 写成当前账务主真相。slide-01 已让 PostgreSQL 成为 local production data canonical truth；slide-02 已让 RuntimeEnvironment 可见套餐、订阅和权益状态来自 typed Portal lab API client。

最近已通过 landing gate 的产品闭环是 `feat/v22-slide-02-portal-api-real-data`，landed commit 为 `5c3f83585c78a5fdb48ed31741719658a4e8d1d6`。该分支把 RuntimeEnvironment 的套餐、subscription 和 entitlement 状态接到 typed Portal lab API client，并移除 lab API 的 `active-missing-ui` 裁决；当前 closeout 已把 active baton 推进到 slide-03。
...
- Account、wallet、balance、frozen amount、ledger 和 audit 语义仍需 owner-scoped 本地闭环。
...
| Product engineering loop | pre-cloud product slides must run as an active baton, not permanent planning prose | `tests/fixtures/v22/goal-current.json` `product_engineering_loop` `precloud-product-slides-closure` + manifest `product-engineering-loop` suite | slide-01 和 slide-02 已 landed；当前 open baton 是 slide-03，open 明细只在 current machine fixture 临时存在 | run slide-03; then continue slide-04..slide-09 in order | 9 个产品 slide 全部闭合后 collapse to history summary and next cursor，active truth 不保 slide 明细、per-slide docs、compat layer 或 shadow archive | `node tests/contract/contract-test-v22-product-engineering-loop-index.mjs` |

## Current Development Lines

### current-stage-current-cursor

Current evidence: latest landed product closeout is `5c3f83585c78a5fdb48ed31741719658a4e8d1d6`; current machine cursor is `leaf-account-wallet-billing-closure`.

Gap: slide-01 data truth and slide-02 Portal API real data wiring are complete, but slide-03 account/wallet/billing closure is still gated and not implemented.

Next action: implement slide-03 so balance, frozen amount, ledger and audit state are owner-scoped and locally queryable, while retaining slide-01/slide-02 regressions as guards.

Done when: readers can distinguish landed slide-01/slide-02 facts from unfinished slide-03 account/wallet/billing work.
...
Current evidence: MedOPL is the SaaS control plane and managed delivery platform for clean One Person Lab; Portal owns account, workspace, balance, files, billing and trace surfaces while OPL owns scientific execution inside the workbench. The active baton is `slide-03-account-wallet-billing`.

Gap: product slides must keep the user loop visible instead of collapsing the product back into architecture or cloud-console language. slide-01 and slide-02 have landed; slide-03 still needs to close owner-scoped account, wallet, billing ledger, frozen amount and audit semantics.
