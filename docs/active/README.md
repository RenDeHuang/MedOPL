...
- `package-d-vpc-deploy-runner-plan`: Commercial stabilization no longer treats the current WSL session as the Package D execution environment. The target is a dedicated VPC deploy runner named `medopl-v22-deploy-runner` in `na-siliconvalley` / `medopl-vpc` / `medopl-private-a`; the provisioning checklist fixes Ubuntu LTS or TencentOS, 2C4G, 50GB disk and public IP disabled by default, with SSH only as a temporary operator-IP exception. The security group checklist keeps inbound minimal and outbound limited to TCR, TKE API private endpoint and PostgreSQL `10.66.0.21:5432`; required tools are `git`, `node/npm`, `go`, `docker` and `kubectl`; runner-local secret files stay at `/home/dev/.secrets/medopl/v22/package-d-deploy.env`, `/home/dev/.secrets/medopl/v22/portal-runtime.env` and `/home/dev/.secrets/medopl/v22/kubeconfig-package-d-deploy`. Runner validation order is `docker version`, `kubectl version --client`, TCR login preflight, Kubernetes API connectivity preflight, PostgreSQL ledger canary and Package D combined preflight. PostgreSQL and TKE API stay private, SSH is limited to operator IP or cloud assistant / console access, and runner state must not enter git. This is a checklist / implementation plan only: no CVM, Tencent mutation, kubectl, deploy, build/push or Package D execution has run.
