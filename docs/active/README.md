...
| latest repo closeout | `recovery/platform-v22-trunk` / `75fab6bfa8e0fa0ce095fdf58605b4c698a5cf41` |
Current summary: 当前主线是 MedOPL 作为产品控制面，为 OPL-Webui 的 `runtime_required` 任务提供 runtime gate、runtime/storage 开通、node pool projection、billing/freeze/audit/release 和 storage destroy intent。OPL-Webui 仍然负责登录后的 ordinary chat 和用户交互入口；MedOPL 不负责 OPL 科研能力质量。最新 local product RC 是 `feat/v22-local-product-rc-single-flow` / `4f195820dcb351030cb1064172bfec2062abc279`，已把 `runtime gate -> upload file -> fileRef -> run -> artifact -> billing/audit -> release stop billing -> explicit storage destroy` 收口进 golden smoke。当前 trunk `75fab6bfa8e0fa0ce095fdf58605b4c698a5cf41` 保持 Portal UI/UX production gates：root/frontend 生产依赖 audit、浏览器可访问性回归、release owner readiness、observability / production deploy receipt 和 canary proof 都必须在生产 claim 前满足。新鲜 `npm run cloud:rollout:availability` 证据显示 `https://portal.medopl.cn/healthz` 和 `/readyz` 返回 MedOPL Go backend JSON 200，因此旧 public Go health/ready routing blocker 已从 current blocker 中清退。provider-key live blocker 的最近源修复已落到 Go production 默认 provider secret root 和 Kubernetes deploy shape：生产 Pod 显式使用 `/tmp/medopl-runtime/provider-secrets`，并通过 writable `emptyDir` 提供运行态目录。Release Image run `27969071518` 和 Cloud Rollout run `27969132092` 已把 `b362010` 镜像部署到 production-canary 并通过 deploy receipt；live_test 已越过 provider-key、run_task artifactRef 和 fetch_artifact 后停在 billing summary 顶层 `runCount` / `ledgerCount` 缺失。本地 Go API 已补 `BillingSummary.runCount` / `ledgerCount` consumer contract、handler/service tests 和实现；Release Image run `27969907029` 已 build `b61a989`，Cloud Rollout run `27969980310` 已把 `b61a989` rollout 到 production-canary Pod，但在 deploy receipt 和 live_test 前因 post-rollout public `healthz` probe 单次检查失败而 fail closed。当前源修复把 rollout public health probe 改为有界重试，同时继续要求 MedOPL Go backend JSON；Release Image run `27970975355` build `e4306f2`，Cloud Rollout run `27971025234` 已越过 rollout apply、deploy receipt preflight 和 deploy receipt lane，随后 live_test 在 `release_runtime` 因公开响应缺顶层 `auditEventId` fail closed。本地 API contract、handler/service tests 和 Go `ReleaseResult` 已补 `auditEventId`。完整 OPL-Webui runtime-required E2E、cloud release candidate receipt manifest、release mutation 和 production complete 仍未证明。下一步仍是重新 build/rollout/live_test，完成 `login/session or canary token -> runtime_required -> MedOPL runtime/storage open -> upload -> fileRef -> run -> artifact -> billing/audit -> release/stop billing -> optional storage destroy`，raw workflow evidence、secret、kubeconfig、cloud payload、runtime artifact、用户上传文件和 agent transcript 仍不得进入 git.
