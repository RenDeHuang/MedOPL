...
| latest repo closeout | `feat/goal-g3-business-closure-post-validation-artifact-go-setup` / `76bd2d7c3adb28355f077fe2e06b347c0debaa4b` |
Current summary: 当前主线是 MedOPL 作为产品控制面，为 OPL-Webui 的 `runtime_required` 任务提供 runtime gate、runtime/storage 开通、node pool projection、billing/freeze/audit/release 和 storage destroy intent。OPL-Webui 仍然负责登录后的 ordinary chat 和用户交互入口；MedOPL 不负责 OPL 科研能力质量。最新 local product RC 是 `feat/v22-local-product-rc-single-flow` / `4f195820dcb351030cb1064172bfec2062abc279`，已把 `runtime gate -> upload file -> fileRef -> run -> artifact -> billing/audit -> release stop billing -> explicit storage destroy` 收口进 golden smoke。The latest repo/gate closeout is feat/goal-g3-business-closure-post-validation-artifact-go-setup at 76bd2d7c3adb28355f077fe2e06b347c0debaa4b. Cloud SaaS productization has landed G1 account/recharge/plan productization and G2 OPL-Webui runtime_required consumer refs projection; the next cursor is `goal-g3-real-cloud-runtime-storage-proof`, which is a cloud-execute slice and must use the existing authorization boundary before claiming real runtime/storage proof. The prior feature slice `goal-commercial-gap6-local-controlled-business-closure` fixes gap6 as local/controlled commercial business closure: account approved, plan selected, balance sufficient, quota available, purchase/credit/open, `resource_preauth_freeze`, upload/run/artifact, debit/ledger/audit, release/destroy/stop billing, `return_to_opl_task` and insufficient balance fail-closed are covered by Go backend commercial ledger E2E, billing reconciliation, runtime_required action contract and return-to-OPL contract. Cost ceiling for this business claim comes from account balance, plan hold amount and quota, not `MEDOPL_CANARY_COST_CEILING_USD`。`MEDOPL_CANARY_*` may remain only as Cloud Rollout operations safety config while active callers consume it; it is not business admission, recharge, quota, cost ceiling or billing truth, and it is not a gap6 blocker. The existing authorized Cloud Rollout manifest can still claim only its scoped cloud release candidate / production-complete candidate evidence; cloud deployment proof for the commercial closure, external PSP settlement, full OPL-Webui resume implementation, unscoped production complete, multi-region/SLA, enterprise compliance, ongoing authorization and unobserved tenants/resources remain unclaimed. Raw workflow evidence、secrets、kubeconfig、cloud payloads、runtime artifacts、uploaded files 和 transcripts 继续留在 `.runtime` 或外部 evidence store，不进入 git。
