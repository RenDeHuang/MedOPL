...
| current blocker | Package D is deployed inside TKE and in-cluster HTTP reachability has passed: the runner image and four service images are published, `portal-frontend`, `medopl-go-backend`, `opl-web-gateway` and `opl-runtime-bridge` Deployments are ready `1/1`, all four ClusterIP Services return HTTP `200`, and redacted evidence is recorded at `.runtime/package-d-production-deploy/pdrun-20260616-004/{deploy,smoke,rollback}-redacted.json` plus `.runtime/package-d-service-reachability/psr-20260617-004/{readonly-service-reachability,smoke-job-manifest}-redacted.json`. Production launch Gap 01 is now repo-native and local-gated: `tests/support/cloud-prework/production-launch-bootstrap-runner.js` proves first admin identity, tenant bootstrap, workspace seed, providerKeyRef-only boundary, local-RC-default separation, redacted evidence shape and external-access block without reading secrets/kubeconfig or executing cloud/deploy work; evidence path is `.runtime/production-launch-bootstrap/<runid>/bootstrap-contract-redacted.json`. This is still contract-only production bootstrap readiness, not formal launch. External/public user access remains `blocked` until the multi-tenant minimum closes. The next unique gap is `production-launch-gap-02-portal-backend-package-c-live-operation-contract`: Portal -> Go backend -> Package C operation contract/local gate before PostgreSQL ledger live write/read, billing/audit/quota closure, workspace lifecycle, canary/rollback or external access. |
