...
| current blocker | `commercial_business_closure_deploy_verification_pending_for_upload_billing_writeback_fix`. Release Image run `28310282648` built image tag `235ea17`; Cloud Rollout run `28310293188` reached rollout apply and deploy receipt, and public availability passed, but that older image failed closed at `upload_file` with `errorCategory=billing_event_write_failed` / `dbOperationStage=save_billing_event`. The source fix is landed at `34b0e3070357504b475a0bee2ff6bd4de0af23df`; the remaining proof is authorized Release Image / Cloud Rollout / OPL-Webui consumer canary rerun for the same commercial business flow. |
| next owner | `MedOPL Platform` owns account approval, credit/recharge, plan/balance/quota, billing ledger, statement reconciliation, runtime/storage lifecycle and audit truth |
...
| latest repo closeout | `goal-commercial-upload-file-billing-writeback-fix` / `34b0e3070357504b475a0bee2ff6bd4de0af23df` |
Current summary: 当前主线是 MedOPL 作为商业 SaaS 资源控制面，为 OPL-Webui 的 `runtime_required` 任务完成 `account prepare -> account approval -> credit -> plan -> open runtime/storage -> upload/run/artifact -> billing ledger -> statement reconciliation -> release/destroy/stop billing`。OPL-Webui 仍然负责登录后的 ordinary chat 和用户交互入口；MedOPL 不负责 OPL 科研能力质量。最新 local product RC 是 `feat/v22-local-product-rc-single-flow` / `4f195820dcb351030cb1064172bfec2062abc279`，已把 `runtime gate -> upload file -> fileRef -> run -> artifact -> billing/audit -> release stop billing -> explicit storage destroy` 收口进 golden smoke。The latest repo/gate closeout is goal-commercial-upload-file-billing-writeback-fix at 34b0e3070357504b475a0bee2ff6bd4de0af23df. That closeout fixed `upload_file -> save_billing_event` to use canonical runtime ledger tenant / billing attribution identity before any account fallback, while keeping billing/audit fail-closed. Release Image run `28310282648` built image tag `235ea17`; Cloud Rollout run `28310293188` applied the older deployment and deploy receipt, but the post-deploy OPL-Webui consumer canary failed closed at `upload_file -> save_billing_event`. The current product proof gap is the authorized deployment rerun for the landed source fix, not deployment operations confirmation governance. Cost ceiling for business claims comes from account balance, plan hold amount and quota. Cloud Rollout workflow inputs and GitHub production environment deployment gate remain deployment operations safety only; they are not business admission, recharge, quota, cost ceiling, billing truth or current product cursor. External PSP settlement, full OPL-Webui resume implementation, unrestricted full production for all users/tenants, multi-region/SLA, enterprise compliance, ongoing authorization and unobserved tenants/resources remain unclaimed. Raw workflow evidence、secrets、kubeconfig、cloud payloads、runtime artifacts、uploaded files 和 transcripts 继续留在 `.runtime` 或外部 evidence store，不进入 git。
...
- `goal-commercial-runtime-storage-billing-business-closure`: active cursor. It must keep the commercial business flow centered on account prepare, account approval, credit, plan, open runtime/storage, billing ledger, statement reconciliation, release/destroy and stop billing. `goal-commercial-upload-file-billing-writeback-fix` is landed at `34b0e3070357504b475a0bee2ff6bd4de0af23df`; the next minimum evidence slice is authorized Release Image / Cloud Rollout / OPL-Webui consumer canary rerun for that same commercial business flow, without bypassing billing/audit or turning upload into a fake success.
