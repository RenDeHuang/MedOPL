...
| latest repo closeout | `recovery/platform-v22-trunk` / `8dfeca42bdec61e37c789953cc06d3980f8e0d20` |
Current summary: 当前主线是 MedOPL 作为产品控制面，为 OPL-Webui 的 `runtime_required` 任务提供 runtime gate、runtime/storage 开通、node pool projection、billing/freeze/audit/release 和 storage destroy intent。OPL-Webui 仍然负责登录后的 ordinary chat 和用户交互入口；MedOPL 不负责 OPL 科研能力质量。最新 local product RC 是 `feat/v22-local-product-rc-single-flow` / `4f195820dcb351030cb1064172bfec2062abc279`，已把 `runtime gate -> upload file -> fileRef -> run -> artifact -> billing/audit -> release stop billing -> explicit storage destroy` 收口进 golden smoke。The latest repo/gate closeout is recovery/platform-v22-trunk at 8dfeca42bdec61e37c789953cc06d3980f8e0d20. Phase A PostgreSQL Business Persistence RC and Phase E billing idempotency hardening are source-landed and live-rollout verified for the current authorized canary path. Phase F operational stability gate is committed on authoring branch `933b9f5` pending trunk landing: production-complete candidate manifests must carry 14 criteria, adding `soak_test_receipt`, `concurrency_pressure_receipt`, `rollback_drill_receipt`, `continuous_canary_monitoring_receipt`, `alerting_receipt` and `final_release_decision_receipt` as `.runtime` pointer evidence. The latest remote Cloud Rollout manifest from run `28057493445` is historical eight-criteria evidence and must be regenerated after landing; `MEDOPL_ALERT_ROUTE_REF` is required in the GitHub production environment for the alerting receipt. This can claim local contract/runner/workflow enforcement only until a fresh Release Image / Cloud Rollout writes the new redacted manifest. It still does not claim unscoped production complete, multi-region/SLA, enterprise compliance, ongoing authorization or broader unobserved tenants/resources. Raw workflow evidence, secrets, kubeconfig, cloud payloads, runtime artifacts, uploaded files and transcripts stay outside git.
...
- `cloud-future-authorized`: keeps Package C dry-run plan, TKE bootstrap preflight local gates and the single `cloud:goal -- --operation <operation_class>` authorization-pack runner plumbing. Goal B may execute local no-cloud plans; Goal A/C/D/E have accepted owner receipt pointers; Goal F has historical remote cloud RC / production-complete candidate evidence through Release Image run `28057445074`, Cloud Rollout run `28057493445` and the old eight-criteria redacted receipt manifest. Phase F now requires a fresh rollout manifest with 14 criteria before `verify:production-complete-candidate` can pass under current truth. Final production complete release decision remains scoped to the manifest and still cannot expand into multi-region/SLA/compliance claims.
