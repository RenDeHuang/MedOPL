...
| latest repo closeout | `feat/v22-package-backend-migrations-for-rollout` / `fc258012a862234d153057594b263281b46c1152` |
Current summary: 当前主线是 MedOPL 作为产品控制面，为 OPL-Webui 的 `runtime_required` 任务提供 runtime gate、runtime/storage 开通、node pool projection、billing/freeze/audit/release 和 storage destroy intent。OPL-Webui 仍然负责登录后的 ordinary chat 和用户交互入口；MedOPL 不负责 OPL 科研能力质量。最新 local product RC 是 `feat/v22-local-product-rc-single-flow` / `4f195820dcb351030cb1064172bfec2062abc279`，已把 `runtime gate -> upload file -> fileRef -> run -> artifact -> billing/audit -> release stop billing -> explicit storage destroy` 收口进 golden smoke。The latest repo/gate closeout is feat/v22-package-backend-migrations-for-rollout at fc258012a862234d153057594b263281b46c1152. Phase A PostgreSQL Business Persistence RC is now source-landed: production Go backend fails closed without `DATABASE_URL`, selects a Postgres control-plane store in production, keeps local/test on memory, persists account/provider/runtime/storage/file/run/artifact/audit/release metadata through the store, and uses typed `business_accounts` / `credit_events` tables for idempotent and atomic credit balance updates. This is source/local gate evidence only; live DB rollout, normalized typed tables for every projection, billing summary reconciliation, long-running concurrency, and production complete remain future phases. 当前 trunk 保持 Portal UI/UX production gates：root/frontend 生产依赖 audit、浏览器可访问性回归、release owner readiness、observability / production deploy receipt、S-level UI polish gate、production-complete owner gate 和 canary proof 都必须在生产 claim 前满足。Release Image run `28012858200` built `8753e2b`，Cloud Rollout run `28013437604` deployed image tag `8753e2b` and passed rollout apply, Portal public entry, HTTP-to-HTTPS redirect, Go health/ready availability, OPL-Webui consumer canary, production receipt manifest, and `verify:cloud-release-candidate`; the redacted manifest has seven owner receipts, 12 lifecycle sections, `claim=cloud_release_candidate`, `state=complete`, and sha256 `3d2b6e8ffe3601ccc3f0489e59079657af0fe2f9972cf9f34713d170903cd85b`. This closes the Portal user entry, HTTP redirect, cloud gate coverage and current authorized E2E canary gaps as cloud RC evidence only. It still does not claim production complete, multi-region/SLA, enterprise compliance, ongoing authorization, broader unobserved tenants/resources, or long-running stability. Raw workflow evidence, secrets, kubeconfig, cloud payloads, runtime artifacts, uploaded files and transcripts stay outside git.
