...
- `cloud-future-authorized`: keeps Package C dry-run plan, TKE bootstrap preflight local gates and the single `cloud:goal -- --operation <operation_class>` authorization-pack runner plumbing. Goal B may execute local no-cloud plans; Goal A/C/D/E have accepted owner receipt pointers; Goal F has prior remote cloud RC evidence through Release Image run `27911002260` and Cloud Rollout run `27911028428`; the latest hardened rollout on run `27912789001` correctly fails closed before any cloud RC claim because `portal.medopl.cn` is still routed to static nginx HTML instead of Go control-plane JSON.
