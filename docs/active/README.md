...
| latest repo closeout | `feat/v22-goal-d-runtime-provisioning-receipt` / `f3644aeb2c7e736da87297762d86639ca98779b5` |
Current summary: 当前主线是 MedOPL 作为产品控制面，为 OPL-Webui 的 `runtime_required` 任务提供 runtime gate、runtime/storage 开通、node pool projection、billing/freeze/audit/release 和 storage destroy intent。OPL-Webui 仍然负责登录后的 ordinary chat 和用户交互入口；MedOPL 不负责 OPL 科研能力质量。最新 local product RC 是 `feat/v22-local-product-rc-single-flow` / `4f195820dcb351030cb1064172bfec2062abc279`，已把 `runtime gate -> upload file -> fileRef -> run -> artifact -> billing/audit -> release stop billing -> explicit storage destroy` 收口进 golden smoke；最新 repo/gate closeout 是 `feat/v22-goal-d-runtime-provisioning-receipt` / `f3644aeb2c7e736da87297762d86639ca98779b5`，Phase 3 已实跑 `cloud:rollout:dry-run`、MedOPL GitHub/TKE rollout shape gate 和 cloud lane，证明 repo-native Kubernetes/GitHub rollout shape、TCR image allowlist、TKE namespace/nodeSelector、self-hosted Tencent runner requirement 和 no-secret/no-kubectl dry-run 边界；Phase 4 GitHub/TKE external preconditions 已由外部配置确认：GitHub workflows active、自托管 Tencent runner 在线且带 `self-hosted/Linux/X64/tencent-cloud/medopl` 标签，production environment 存在；TKE node `10.66.0.42` Ready 且带 `medopl.cn/workload=medopl` / `medopl.cn/webui=true`，namespace `medopl` Active。Goal A readonly inventory 已通过显式 secret-file 和 `tencent-official-sdk-readonly` 路径执行，只写 `.runtime/v22-cloud-authorization/run-v22-001/readonly_inventory.json` redacted pointer；Goal C storage lifecycle 已通过 authorized COS probe 执行，证明 workspace probe `PutObject -> HeadObject -> DeleteObject` 可用，并写入 `storage_owner_receipt` / `release_owner_receipt` pointer；Goal D tenant runtime provisioning 已通过 authorized TKE runner 执行，观察到配置的 cluster / node pool，并写入 `runtime_owner_receipt` pointer；Goal E OPL-Webui consumer canary 已通过 authorized `live_test` runner 执行，观察到 `https://opl.medopl.cn` 与 `https://portal.medopl.cn` 均为 HTTPS 200，并写入 `opl_webui_consumer_receipt` pointer。该 proof 只证明 public HTTPS consumer canary、runtime foundation observation 和 storage/runtime/consumer owner receipts 已接受，不证明 billing/audit writeback、kubectl、deploy、build/push 或 production complete。`verify:cloud-release-candidate` 仍按设计 fail closed：Goal A/C/D/E 的单项 receipt pointer 不能替代完整 receipt manifest，package gate 仍缺 billing/audit/deploy owner receipts 和完整 lifecycle sections。后续每个 gap 仍允许 push feature branch，并允许 fresh landing gate 通过后 `ff-only merge trunk`、`push trunk`、`post-push verify` 和 `tombstone cleanup`，但不放开真实云 mutation、deploy、build/push、kubectl、live-test 或 production claim。real local product RC != cloud 上线，cloud-deployable RC != production complete。生产 proof 不能由 local/cloud RC 或 stub runner 自动升级，必须显式补齐 runtime / storage / billing / audit / release / OPL-Webui consumer / production deploy owner receipts，且 receipt manifest 必须绑定 `operation_class`、`runner_id` 和当前 authorization run。历史 Package D deploy execution evidence 只保留为 provenance，不是当前 active runner 或新授权。当前 active cloud surface 保留 readonly inventory、Package C dry-run plan、TKE bootstrap preflight、GitHub cloud rollout shape 和 authorization-pack 单一 `cloud:goal -- --operation <operation_class>` map；`cloud:goal:preflight` 只做 Goal A-F 的 env / path / runner readiness 检查，不读取 secret、不调用云、不写 `.runtime`；Goal B dry-run plan 可本地执行且不触云，Goal C storage lifecycle 已有 accepted storage/release owner receipt pointers，Goal D tenant runtime provisioning 已有 accepted runtime owner receipt pointer，Goal E OPL-Webui consumer canary 已有 accepted consumer receipt pointer；Goal F 仍有可接外部 runner 的 fail-closed 接口，但在缺少 env、runner 路径、accepted receipt status、redacted pointer evidence 或 owner receipt 时仍阻断。旧 production launch、Package D deploy/external access、CLB diagnostics 和 Package C live canary 只作为 history / archive / runtime provenance 存在，不再是 active runner、active test 或 current gap。下一 phase 是 Goal F real runner receipts；它需要 billing/audit writeback、build/push、kubectl、deploy、live-test 和 production receipt manifest，不得写成 production complete。
...
- `cloud-future-authorized`: keeps Package C dry-run plan, TKE bootstrap preflight local gates and the single `cloud:goal -- --operation <operation_class>` authorization-pack runner plumbing. Goal B may execute local no-cloud plans; Goal C, Goal D and Goal E now have accepted owner receipt pointers; Goal F must fail closed until required env, real runner implementation and owner receipts exist.
