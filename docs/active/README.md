...
| latest repo closeout | `recovery/platform-v22-trunk` / `eeab7fcf5927c522d79e05a7d28360dc6e3d5560` |
Current summary: 当前主线是 MedOPL 作为产品控制面，为 OPL-Webui 的 `runtime_required` 任务提供 runtime gate、runtime/storage 开通、node pool projection、billing/freeze/audit/release 和 storage destroy intent。OPL-Webui 仍然负责登录后的 ordinary chat 和用户交互入口；MedOPL 不负责 OPL 科研能力质量。最新 local product RC 是 `feat/v22-local-product-rc-single-flow` / `4f195820dcb351030cb1064172bfec2062abc279`，已把 `runtime gate -> upload file -> fileRef -> run -> artifact -> billing/audit -> release stop billing -> explicit storage destroy` 收口进 golden smoke。The latest repo/gate closeout is recovery/platform-v22-trunk at eeab7fcf5927c522d79e05a7d28360dc6e3d5560. 当前 trunk 保持 Portal UI/UX production gates：root/frontend 生产依赖 audit、浏览器可访问性回归、release owner readiness、observability / production deploy receipt、S-level UI polish gate、production-complete owner gate 和 canary proof 都必须在生产 claim 前满足。GitHub v22 verify run `27974444437` passed for Portal production UI gate hardening. Release Image run `27972408990` built `485afcf`，Cloud Rollout run `27972458642` passed rollout apply, deploy receipt, OPL-Webui consumer canary, availability probe, production receipt manifest, and `verify:cloud-release-candidate`; the redacted manifest has seven owner receipts, 12 lifecycle sections, `claim=cloud_release_candidate`, `state=complete`, and sha256 `43d6e9de0915190b552e3d0bcf5f9b6fd319953c986b4ad2e998c4c97f976ad2`. This closes the release `auditEventId` canary blocker as cloud RC evidence only. It still does not claim production complete, multi-region/SLA, enterprise compliance, ongoing authorization, or broader unobserved tenants/resources. Raw workflow evidence, secrets, kubeconfig, cloud payloads, runtime artifacts, uploaded files and transcripts stay outside git.
