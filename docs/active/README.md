...
| latest repo closeout | `chore/v22-goal-f-cloud-rc-closeout` / `db25f3405ca3711e628dc19bbaed720a8e60941a` |
Current summary: 当前主线是 MedOPL 作为产品控制面，为 OPL-Webui 的 `runtime_required` 任务提供 runtime gate、runtime/storage 开通、node pool projection、billing/freeze/audit/release 和 storage destroy intent。OPL-Webui 仍然负责登录后的 ordinary chat 和用户交互入口；MedOPL 不负责 OPL 科研能力质量。最新 local product RC 是 `feat/v22-local-product-rc-single-flow` / `4f195820dcb351030cb1064172bfec2062abc279`，已把 `runtime gate -> upload file -> fileRef -> run -> artifact -> billing/audit -> release stop billing -> explicit storage destroy` 收口进 golden smoke；最新 repo/gate closeout 是 `chore/v22-goal-f-cloud-rc-closeout` / `db25f3405ca3711e628dc19bbaed720a8e60941a`。Goal F cloud release candidate wiring 已通过 `feat/v22-goal-f-production-receipt-manifest` / `90a89b75b2a4613719ce9c424367fb35ed51495c` 进入 trunk；Release Image run `27908370667` 在 Goal F head 上成功，Cloud Rollout run `27908384471` 在 Goal F head 上成功并上传 `medopl-production-receipt-manifest` redacted artifact，artifact 内含七类 owner receipt 和十二个 lifecycle section；当前 trunk `db25f3405ca3711e628dc19bbaed720a8e60941a` 的 GitHub `v22 verify` run `27909015808` 已通过。`npm run verify` 在当前 trunk 通过；`npm run verify:cloud-release-candidate` 仍是 evidence-dependent gate：没有 `.runtime/v22-cloud-authorization/run-v22-001/receipt-manifest.json` 时必须 fail closed，下载 redacted manifest artifact 到该路径后可通过。该 proof 只允许 claim cloud release candidate，不允许 claim production complete、multi-region production、SLA、compliance 或完整 OPL-Webui 用户链路。后续必须在当前 trunk 重新触发 Release Image + Cloud Rollout，把 cloud RC 证据绑定到最新 trunk；下一产品 gap 是 Goal G full OPL-Webui runtime-required E2E canary：`login/session -> runtime_required -> MedOPL runtime/storage open -> upload -> fileRef -> run -> artifact -> billing/audit -> release/stop billing -> optional storage destroy`。raw workflow evidence、secret、kubeconfig、cloud payload、runtime artifact、用户上传文件和 agent transcript 仍不得进入 git。
