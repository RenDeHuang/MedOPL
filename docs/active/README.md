...
| current blocker | Package D is deployed inside TKE and in-cluster HTTP reachability has passed: the runner image and four service images are published, `portal-frontend`, `medopl-go-backend`, `opl-web-gateway` and `opl-runtime-bridge` Deployments are ready `1/1`, all four ClusterIP Services return HTTP `200`, and redacted evidence is recorded at `.runtime/package-d-production-deploy/pdrun-20260616-004/{deploy,smoke,rollback}-redacted.json` plus `.runtime/package-d-service-reachability/psr-20260617-004/{readonly-service-reachability,smoke-job-manifest}-redacted.json`. Production launch Gap 01 is repo-native and local-gated through `tests/support/cloud-prework/production-launch-bootstrap-runner.js`. Gap 02 is repo-native and local-gated through `tests/support/cloud-prework/production-launch-operation-runner.js`. Gap 03 is repo-native and local-gated through `tests/support/cloud-prework/production-launch-ledger-runner.js`. Gap 04 is repo-native and local-gated through `tests/support/cloud-prework/production-launch-commercial-ledger-runner.js`. Gap 05 is now repo-native and local-gated through `tests/support/cloud-prework/production-launch-workspace-lifecycle-runner.js`: it proves unauthorized fail-closed behavior, workspace suspend/resume/delete request shape, ResourceBinding lifecycle linkage, CloudOperation lifecycle linkage, billing stop/resume/finalization linkage, audit linkage, quota release/restore linkage, operation idempotency, providerKeyRef-only boundary, rollback/cleanup redacted evidence shape and local dry-run repository versus future production PostgreSQL boundary without reading secrets/kubeconfig/DB password or connecting to PostgreSQL, Kubernetes, deploy, build/push, Tencent mutation or Package C live work; evidence path is `.runtime/production-launch-workspace-lifecycle/<runid>/workspace-lifecycle-contract-redacted.json`. This is still contract-only launch readiness, not formal launch. External/public user access remains `blocked` until the multi-tenant minimum closes. The next unique gap is `production-launch-gap-06-production-canary-rollback-evidence-contract`: production smoke / canary / rollback / cleanup evidence contract before external access. |
