...
- `cloud-future-authorized`: keeps Package C dry-run plan, TKE bootstrap preflight local gates and `cloud:goal:*` authorization-pack runner plumbing. Goal B may execute local no-cloud plans; Goal C-F must fail closed until required env, real runner implementation and owner receipts exist.
...
| production goal runner plan | `npm run cloud:authorized:plan`; `npm run cloud:goal:preflight`; `npm run cloud:goal:dry-run-plan`; `node tests/cloud/cloud-test-v22-cloud-authorized-executor.mjs` |
...
- 不能把 `cloud:goal:dry-run-plan` 写成 runtime/storage 已创建、kubectl 已执行、image 已 push、deploy/live-test 已完成或 production complete。
