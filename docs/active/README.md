...
| current blocker | Package D is deployed inside TKE and in-cluster HTTP reachability has passed: the runner image and four service images are published, `portal-frontend`, `medopl-go-backend`, `opl-web-gateway` and `opl-runtime-bridge` Deployments are ready `1/1`, all four ClusterIP Services return HTTP `200`, and redacted evidence is recorded at `.runtime/package-d-production-deploy/pdrun-20260616-004/{deploy,smoke,rollback}-redacted.json` plus `.runtime/package-d-service-reachability/psr-20260617-004/{readonly-service-reachability,smoke-job-manifest}-redacted.json`. This proves the platform control-plane can run inside TKE, but MedOPL's target is a stable `platform-provisioned / customer-dedicated` multi-tenant SaaS managed OPL workbench, not a temporary page trial. The repo-native Production Launch Goal / Gap Map now lives in `tests/fixtures/v22/goal-current.json` as the machine cursor derivative: it audits auth / identity / admin bootstrap, tenant model, workspace provisioning, Portal -> backend -> Package C flow, Package C cloud operation state machine, Tencent TKE tenant node pool lifecycle, ResourceBinding PostgreSQL ledger, billing ledger, audit ledger, quota enforcement, workspace suspend / resume / delete, external access / Ingress / HTTPS, production smoke / canary, rollback / cleanup and observability / redaction evidence. Status summary: Package D platform service baseline is `deployed`; in-cluster service reachability and Package C one-off tenant node pool lifecycle are `preflight-passed`; Go backend tenant/workspace/schema/control-plane and Package C state-machine surfaces are `implemented` or `contract-only`; PostgreSQL ledger live write/read, billing/audit ledger closure, quota enforcement and workspace lifecycle are not production-proven; external/public user access is `blocked` until the multi-tenant minimum closes. Ordered launch phases are: 0 TKE platform service baseline, 1 production control-plane bootstrap minimum, 2 workspace provisioning to Package C live operation, 3 commercial ledger and lifecycle closure, 4 production smoke/canary/rollback/evidence, 5 external access after the minimum SaaS closure. The next unique gap is `production-launch-gap-01-auth-tenant-workspace-bootstrap-contract`: first admin identity bootstrap, tenant bootstrap and workspace seed contract/local gate before Portal exposure, Package C production wiring, billing/audit/quota closure or external access. |
