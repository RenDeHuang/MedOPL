...
Current summary: 当前主线是 MedOPL 作为产品控制面，为 OPL-Webui 的 `runtime_required` 任务提供 runtime gate、runtime/storage 开通、node pool projection、billing/freeze/audit/release 和 storage destroy intent。OPL-Webui 仍然负责登录后的 ordinary chat 和用户交互入口；MedOPL 不负责 OPL 科研能力质量。最新 local product RC 是 `feat/v22-local-product-rc-single-flow` / `4f195820dcb351030cb1064172bfec2062abc279`，已把 `runtime gate -> upload file -> fileRef -> run -> artifact -> billing/audit -> release stop billing -> explicit storage destroy` 收口进 golden smoke。当前 trunk `cbd0d8962a5bf9ab34a1ff0973c2bb9ba5ca99dc` 已通过 GitHub `v22 verify` run `27913303242`，Release Image run `27913362757` 成功并推送 image tag `cbd0d89`；Cloud Rollout run `27913371704` 已执行 runtime/storage/billing/kubectl 前置 receipt lanes 并把 `medopl-control-plane` rollout 到 image `cbd0d89`，但在 `Rollout apply` 的 Go backend JSON health gate fail closed。诊断显示 Kubernetes Ingress `medopl` 的地址是 `lb-7trlq374-v40s3uxfnpb1dazj.clb.usw-tencentclb.com`，Service endpoint 是 `10.66.0.29:8080`，但公网 DNS `portal.medopl.cn` 仍解析到旧静态 nginx CLB `lb-pwv9zgky-yqsc8g20f1o6p3ir.clb.usw-tencentclb.com` / `43.159.159.90`；`/`、`/healthz`、`/readyz` 和 `/api/opl/runtime-gate` 因此仍返回 `text/html`，不是 MedOPL Go backend JSON。因此完整 Goal G E2E canary 仍未通过，也没有新的 cloud release candidate receipt manifest。当前 gate 已加固：rollout apply、availability probe 和 `live_test` 都必须 fail closed，除非 MedOPL public API 完成 `healthz/readyz -> provider-key -> open runtime -> runtime-gate -> upload -> run -> artifact -> billing -> release -> storage destroy` 的公网 API flow。raw workflow evidence、secret、kubeconfig、cloud payload、runtime artifact、用户上传文件和 agent transcript 仍不得进入 git.
