...
| current blocker | Package D is deployed inside TKE and in-cluster HTTP reachability has passed: the runner image and four service images are published, `portal-frontend`, `medopl-go-backend`, `opl-web-gateway` and `opl-runtime-bridge` Deployments are ready `1/1`, all four ClusterIP Services return HTTP `200`, and redacted evidence is recorded at `.runtime/package-d-production-deploy/pdrun-20260616-004/{deploy,smoke,rollback}-redacted.json` plus `.runtime/package-d-service-reachability/psr-20260617-004/{readonly-service-reachability,smoke-job-manifest}-redacted.json`. Production launch Gap 01 through Gap 08b are repo-native and local-gated, and Gap 08c qcloud TLS Secret + Ingress server-side dry-run passed in cloud at HEAD `abbc82f3bbd39d2c659c514476153f36ccb63d9f`. Gap 08c evidence is `.runtime/package-d-external-access-strategy/gap08c-qcloud-ingress-dry-run-001/dry-run-redacted.json`: context `cls-fi097sy4`, namespace `medopl-platform`, `portal-frontend` Service ClusterIP `8080/TCP`, IngressClass `qcloud` / controller `cloud.tencent.com/ingress-controller`, TLS Secret manifest shape passed, TLS Secret server-side dry-run / validation passed, Ingress manifest shape passed, Ingress server-side dry-run passed, and both `medopl-portal-tls` and the Portal Ingress remained `NotFound` after the dry-run. No real mutation occurred and redaction audit passed. Fixed entry parameters remain `PORTAL_HOST_DOMAIN=portal.medopl.cn`, `INGRESS_CLASS=qcloud`, `TLS_SECRET_NAME=medopl-portal-tls`, `EXTERNAL_SMOKE_URL=https://portal.medopl.cn/`, with `TENCENT_SSL_CERT_ID` required only as an authorized env value whose raw value must not enter docs, logs or evidence. External/public user access remains not exposed. The next unique gap is `production-launch-gap-08d-qcloud-tls-secret-ingress-real-mutation-authorization`: a real mutation authorization pack limited to creating/applying `Secret/medopl-portal-tls` and the Portal Ingress for `portal.medopl.cn -> portal-frontend:8080`, plus get/describe verification, rollback/delete plan, DNS post-apply validation and HTTPS smoke; it still must not claim public access complete before post-apply evidence. |
