...
| current blocker | Package D is deployed inside TKE and in-cluster HTTP reachability has passed: the runner image and four service images are published, `portal-frontend`, `medopl-go-backend`, `opl-web-gateway` and `opl-runtime-bridge` Deployments are ready `1/1`, all four ClusterIP Services return HTTP `200`, and redacted evidence is recorded at `.runtime/package-d-production-deploy/pdrun-20260616-004/{deploy,smoke,rollback}-redacted.json` plus `.runtime/package-d-service-reachability/psr-20260617-004/{readonly-service-reachability,smoke-job-manifest}-redacted.json`. Production launch Gap 01 through Gap 07 are repo-native and local-gated through `production-launch-{bootstrap,operation,ledger,commercial-ledger,workspace-lifecycle,canary}-runner.js` and `package-d-external-access-strategy-runner.js`. Gap 07 proves the Portal external access strategy shape, compares admin-only port-forward, internal gateway, Kubernetes Ingress, LoadBalancer Service and HTTPS/domain, records Ingress + HTTPS/domain as the formal launch candidate, keeps admin-only port-forward as administrator acceptance only, and validates required production entry parameters, allowed/forbidden operations, security boundary, rollback/delete ingress plan, smoke plan, redaction evidence, providerKeyRef-only boundary and Portal typed API -> Go fail-closed route trace. This is still strategy/contract readiness, not public access. External/public user access remains not exposed. The next unique gap is `production-launch-gap-08-external-access-dry-run-authorization`: authorized server-side dry-run for the Ingress + HTTPS/domain candidate, without executing mutation or claiming public access complete. |
