...
商业化主链路必须从客户购买意图出发，而不是从云资源对象出发：谁都可以进入 OPL；需要平台托管计算、文件空间、隔离环境、计费和审计时，必须进入 MedOPL。

当前结构收敛后的商业模型是：

- `api_only`: 用户拥有账号、工作空间、OPL 入口、自己的 gflabtoken `providerKeyRef`、文件/任务/结果索引；不购买平台托管计算资源。
- `full_runtime`: 用户在 `api_only` 之上购买平台托管计算、文件空间、任务并发、余额/冻结金额、run/artifact/trace 回流和释放停止计费。
- `customer_dedicated`: 用户在 `full_runtime` 之上购买更强隔离、客户级运行边界、专属审计标签和人工审批/变更窗口。
