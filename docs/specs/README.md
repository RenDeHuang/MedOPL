...
当前 PostgreSQL resource binding ledger 的已落地机器下界在 `services/medopl-go-backend`：domain model 覆盖 `resourceBindingId`、`billingAttributionId`、`serverPlanId`、`workspaceStorageGb`、Tencent cluster / node pool identity、operationId 和 lifecycle status；baseline migration 与 Ent schema 声明 `resource_bindings` / `cloud_operations`。`nodePoolId` 在 requested / creating 阶段可以为空，`nodePoolName` 必须保留目标 tenant node pool 名称；`cloud_tag_support=tke_nodepool_unsupported` 且 `canonical_ownership_source=postgres_resource_binding_ledger`。repository contract 覆盖 create resource binding、append cloud operation event、CreateNodePool 成功后回填 nodePoolId、状态推进、mark released、mark failed 和 mark cleanupRequired；local/memory store 是 dry-run contract store。Package C live runner 现在把 tenant node pool lifecycle 投影为同一 ResourceBinding / CloudOperation 状态机 evidence，并通过注入式 ledger sink 调用同一写入合同，覆盖 requested、creating、created、scaling、ready、releaseRequested、deleting、released、failed 和 cleanupRequired。Package C PostgreSQL sink support 进一步提供 `RUN_MEDOPL_POSTGRES_LEDGER_EXECUTION=1` gate、allowlisted local DB env keys、schema/table/write-permission preflight、参数化写入方法、redacted `.runtime` evidence 和 missing-driver fail-closed 行为；prepare-only 不写业务记录。这个状态机和 sink proof 仍只默认写 `.runtime` 或 local memory；本轮没有读取真实 DB secret、没有执行真实 DB connection，不代表 Portal 开通入口、授权真实 PostgreSQL write、billing/audit ledger、workspace quota 或真实 DB migration 已执行。
...
Package C create/release canary 保留 post-create `TagResources` plan，但 TKE tenant node pool 当前不把 Tencent `tke:nodepool` cloud tags 作为 hard blocker。若 `TagResources` 返回 `InvalidParameter.UnsupportedService` 且指向 `tke:nodepool`，runner 必须记录 non-blocking event 和 `cloudTagSupport=tkeNodePoolUnsupported`，继续后续 scale / observe / release；其它 `TagResources` 错误仍 fail-closed 并只允许清理本次 canary tenant node pool。ownership / billing attribution 的 canonical truth 是 PostgreSQL `resource_bindings` / `cloud_operations` ledger；当前 canary 阶段先写 `.runtime` ResourceBinding / CloudOperation state-machine evidence，后续 production path 才执行 PostgreSQL write。云标签仅作为支持资源层的 reconciliation evidence，不得替代 Portal canonical ledger。
