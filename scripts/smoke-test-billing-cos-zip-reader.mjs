...
async function runBillingPreviewContract() {
  ...
  const listXml = buildListXml(cosZipKey, billZip.length);
  ...
  try {
    ...
    assert.equal(files.files?.[0]?.key, cosZipKey);
    ...
    assert.equal(preview.latestFile?.key, cosZipKey);
    assert.equal(preview.parsedRowCount, 2);
    assert.equal(preview.attributedCount, 1);
    assert.equal(preview.unattributedCount, 1);
    assert.equal(preview.totalCost, 45.67);
    assert.equal(preview.items?.[0]?.resourceOrderId, "order-preview-1");
    assert.equal(preview.items?.[0]?.workspaceId, "workspace-preview-1");
