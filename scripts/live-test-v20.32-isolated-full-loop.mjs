...
const evidenceDir = path.join(repoRoot, ".runtime", "v20.32-isolated-full-loop");
...
async function loadConfiguredEnvFiles() {
  const files = [
    ...envFileList(process.env.V20_32_ENV_FILE),
    ...envFileList(process.env.V20_32_SECRETS_ENV_FILE),
  ];
...
async function writeEvidence(payload) {
  ...
  const safePayload = sanitizeEvidence(payload);
...
function assertFixedHostsAndIsolatedTransport({ portalBaseUrl, oplBaseUrl }) {
  const confirmed = boolEnv("V20_32_ISOLATED_TRANSPORT_CONFIRMED");
  ...
  assert.equal(confirmed, true, "V20_32_ISOLATED_TRANSPORT_CONFIRMED_must_be_true");
  assert(connectHost, "V20_32_CONNECT_HOST_required_for_fixed_host_isolated_transport");
}
...
function makeFixtureIdentity() {
  const slug = `test-v20-32-${Date.now().toString(36)}-${randomBytes(3).toString("hex")}`.slice(0, 44);
  return {
    slug,
    ...
    fileText: [
      "v20.32 isolated full loop fixture",
      `workspaceId=${slug}`,
      `createdAt=${nowIso()}`,
    ].join("\n"),
  };
...
async function uploadWorkspaceFile(config, userCookie, fixture) {
  ...
  const boundary = `----v20-32-${randomBytes(8).toString("hex")}`;
...
async function sendOplMessage(config, launch, bootstrap, fixture) {
  ...
  const payload = await absoluteJson(bootstrap.callbacks.message, {
    method: "POST",
    headers: { "content-type": "application/json", "user-agent": "v20.32-isolated-full-loop" },
    body: JSON.stringify({
      launchToken: launch.launchToken,
      messageId: `${fixture.runId}-message`,
      message: "请回复：v20.32 isolated full loop ok",
      model: "gpt-5.4",
      tokenCount: 32,
    }),
...
async function startOplFileRun(config, launch, bootstrap, fixture) {
  const run = await absoluteJson(bootstrap.callbacks.startRun, {
    ...
    body: JSON.stringify({
      ...
      runId: fixture.runId,
      agentId: "mas",
      toolName: "v20.32-isolated-full-loop",
      source: "v20.32-isolated-full-loop",
...
async function deleteNodePool(config, userCookie, resourceOrderId) {
  assert.equal(boolEnv("V20_32_CONFIRM_DELETE"), true, "V20_32_CONFIRM_DELETE_must_be_true_before_delete");
...
function buildConfig() {
  const portalBaseUrl = trimTrailingSlash(env("V20_32_PORTAL_BASE_URL", "https://portal.medopl.cn"));
  const oplBaseUrl = trimTrailingSlash(env("V20_32_OPL_BASE_URL", "https://opl.medopl.cn"));
  return {
    portalBaseUrl,
    oplBaseUrl,
    expectedBuildTag: env("V20_32_EXPECTED_BUILD_TAG", "opl-v20.32"),
    adminEmail: env("V20_32_ADMIN_EMAIL", env("PORTAL_ADMIN_EMAIL")),
    adminPassword: env("V20_32_ADMIN_PASSWORD", env("PORTAL_ADMIN_PASSWORD")),
    gflabtoken: env("V20_32_GFLABTOKEN", env("GFLABTOKEN")),
    rechargeAmount: positiveIntEnv("V20_32_RECHARGE_AMOUNT", 1000),
    storageGb: positiveIntEnv("V20_32_STORAGE_GB", 10),
    serverPlanId: env("V20_32_SERVER_PLAN_ID", "cpu-2c4g"),
    billingObserveMinutes: Number(env("V20_32_BILLING_OBSERVE_MINUTES", "0")),
    expected120MinMinutes: positiveIntEnv("V20_32_EXPECTED_120MIN_MINUTES", 120),
    timeoutMs: positiveIntEnv("V20_32_HTTP_TIMEOUT_MS", 120_000),
    oplTimeoutMs: positiveIntEnv("V20_32_OPL_TIMEOUT_MS", 240_000),
    provisionTimeoutMs: positiveIntEnv("V20_32_PROVISION_TIMEOUT_MS", 15 * 60_000),
    pollTimeoutMs: positiveIntEnv("V20_32_POLL_TIMEOUT_MS", 15 * 60_000),
    pollMs: positiveIntEnv("V20_32_POLL_MS", 5_000),
  };
}

function assertRequiredConfig(config) {
  const missing = [
    ["V20_32_ADMIN_EMAIL", config.adminEmail],
    ["V20_32_ADMIN_PASSWORD", config.adminPassword],
    ["V20_32_GFLABTOKEN", config.gflabtoken],
...
if (env("RUN_V20_32_FULL_LOOP") !== "1") {
  const payload = {
    ok: true,
    status: "skip",
    reason: "RUN_V20_32_FULL_LOOP_not_enabled",
    requiredEnv: [
      "RUN_V20_32_FULL_LOOP",
      "V20_32_ISOLATED_TRANSPORT_CONFIRMED",
      "V20_32_CONNECT_HOST",
      "V20_32_CONFIRM_DELETE",
      "V20_32_PORTAL_BASE_URL",
      "V20_32_OPL_BASE_URL",
      "V20_32_EXPECTED_BUILD_TAG",
      "V20_32_ADMIN_EMAIL",
      "V20_32_ADMIN_PASSWORD",
      "V20_32_GFLABTOKEN",
      "V20_32_RECHARGE_AMOUNT",
      "V20_32_STORAGE_GB",
      "V20_32_SERVER_PLAN_ID",
      "V20_32_BILLING_OBSERVE_MINUTES",
    ],
    stageOrder,
  };
...
try {
  ...
  const payload = {
    ok: true,
    status: "live",
    contract: "v20.32_isolated_full_loop",
    evidence,
  };
  payload.evidencePath = await writeEvidence(payload);
