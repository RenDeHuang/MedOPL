...
const thisGate = "scripts/smoke-test-v22-opl-style-taxonomy-hard-compaction.mjs";
...
function assertNoForbiddenChangedFiles(files) {
  ...
  for (const file of files) {
    assert(allowedChangedPrefixes.some((prefix) => file.startsWith(prefix)), `hard_compaction_changed_file_outside_docs_scripts:${file}`);
  }
}
...
assert(override.commands.includes("node scripts/smoke-test-v22-docs-taxonomy-skeleton.mjs"), "hard_compaction_override_must_run_taxonomy_gate");
assert(override.commands.includes("node scripts/smoke-test-v22-agent-run-record-gate.mjs"), "hard_compaction_override_must_run_agent_record_gate");
