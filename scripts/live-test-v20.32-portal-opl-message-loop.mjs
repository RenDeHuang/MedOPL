...
const evidenceDir = path.join(repoRoot, ".runtime", "v20.32-portal-opl-message-loop");
const requiredEnv = [
  "PORTAL_LIVE_EMAIL",
...
async function loginPortalSessionCookie(baseUrl, email, password) {
  const retries = positiveIntEnv("V20_32_LOGIN_GATEWAY_RETRIES", 3);
  let lastResult = null;
...
if (env("RUN_V20_32_LIVE") !== "1") {
  const payload = {
    ok: true,
    status: "skip",
    reason: "RUN_V20_32_LIVE_not_enabled",
  };
...
const portalBaseUrl = baseUrl(env("V20_32_PORTAL_BASE_URL", env("PORTAL_LIVE_BASE_URL", "https://portal.medopl.cn")));
const oplBaseUrl = baseUrl(env("V20_32_OPL_BASE_URL", env("OPL_LIVE_BASE_URL", "https://opl.medopl.cn")));
...
const messageText = env("V20_32_LIVE_MESSAGE", "请回复：v20.32 portal->opl live gate ok");
