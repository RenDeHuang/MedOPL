...
const evidenceDir = path.join(repoRoot, ".runtime", "v20.33-production-preacceptance");
const evidenceRecorder = createEvidenceRecorder({ evidenceDir, contract: "v20.33_production_preacceptance" });
...
export const V20_33_PREACCEPTANCE_USER_PAGES = Object.freeze([
...
export const V20_33_PREACCEPTANCE_ADMIN_PAGES = Object.freeze([
...
function assertFixedHosts({ portalBaseUrl, oplBaseUrl, traceBaseUrl }) {
  ...
  if (boolEnv("V20_33_REQUIRE_ISOLATED_TRANSPORT")) {
    assert.equal(boolEnv("V20_33_ISOLATED_TRANSPORT_CONFIRMED"), true, "V20_33_ISOLATED_TRANSPORT_CONFIRMED_must_be_true");
    assert(env("V20_33_CONNECT_HOST"), "V20_33_CONNECT_HOST_required_when_isolated_transport_required");
  }
}
...
async function requestNodeText(url, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.setTimeout(options.timeoutMs || positiveIntEnv("V20_33_PREACCEPTANCE_HTTP_TIMEOUT_MS", 120_000), () => req.destroy(new Error("timeout")));
...
function summarizeExperience(payload) {
  ...
  return {
    ...
    note: "该验收不创建或删除 CVM，不执行模型消息；OPL 回复耗时由 live-test-v20.33-portal-opl-message-loop.mjs 记录。",
  };
}

if (env("RUN_V20_33_PREACCEPTANCE") !== "1") {
  evidenceRecorder.addStage("preacceptance_skip", {
    blockingUser: false,
    userVisibleState: "v20.33 production preacceptance is waiting for explicit RUN_V20_33_PREACCEPTANCE opt-in",
  });
  const payload = {
    ok: true,
    status: "skip",
    reason: "RUN_V20_33_PREACCEPTANCE_not_enabled",
    requiredEnv: [
      "RUN_V20_33_PREACCEPTANCE",
      "PORTAL_LIVE_EMAIL",
      "PORTAL_LIVE_PASSWORD",
      "V20_33_EXPECTED_BUILD_TAG",
    ],
    userPages: V20_33_PREACCEPTANCE_USER_PAGES,
    adminPages: V20_33_PREACCEPTANCE_ADMIN_PAGES,
...
const config = {
  ...
  oplBaseUrl: baseUrl("V20_33_OPL_BASE_URL", "https://opl.medopl.cn"),
  traceBaseUrl: baseUrl("V20_33_TRACE_BASE_URL", "https://trace.medopl.cn"),
...
try {
  ...
  evidenceRecorder.addStage("portal_build", {
    blockingUser: false,
    userVisibleState: "Portal build tag verified",
    latencyMs: evidence.portalBuild.latencyMs,
    buildTag: evidence.portalBuild.buildTag,
  });
  ...
  evidenceRecorder.addStage("portal_login", {
    blockingUser: false,
    userVisibleState: "Portal session established",
  ...
  evidence.apiProbes = await Promise.all(
    [...V20_33_PREACCEPTANCE_USER_PAGES, ...V20_33_PREACCEPTANCE_ADMIN_PAGES].map((item) => apiProbe(config, cookieHeader, item)),
  );
  ...
  for (const item of [...V20_33_PREACCEPTANCE_USER_PAGES, ...V20_33_PREACCEPTANCE_ADMIN_PAGES]) {
    evidence.pages.push(await renderPage(page, config, item));
  }
  evidenceRecorder.addStage("portal_pages", {
    blockingUser: evidence.pages.some((item) => !item.ok || !item.requiredTextFound),
    userVisibleState: "Portal user and admin pages rendered with required business text",
    latencyMs: Math.max(0, ...evidence.pages.map((item) => Number(item.latencyMs || 0))),
  ...
  evidenceRecorder.addStage("portal_opl_jump", {
    blockingUser: !evidence.portalOplJump.ok,
    userVisibleState: "Portal to OPL entry reached the official OPL host",
    latencyMs: evidence.portalOplJump.latencyMs,
    finalHost: evidence.portalOplJump.finalHost,
  });
