...
| `framework:productization-roadmap-required` | Framework | `docs/delivery/README.md`, `changes/active/golden-path-productization-roadmap` | `node tests/contract/contract-test-v22-change-package-lifecycle.mjs`; `node scripts/v22-workflow-gate.mjs review --base origin/recovery/platform-v22-trunk` | local contract proof | Chat memory can define the current productization order. |
