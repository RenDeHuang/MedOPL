...
| `source:node-portal-backend-physical-removal` | Product + Operations | `services/portal/frontend`, `services/medopl-go-backend`, `docs/source/README.md` | `node tests/contracts/contract-test-v22-node-portal-backend-physical-removal.mjs`; `node tests/health/health-check-v22-zero-compat-active-surface-gate.mjs` | local cleanup proof | Node Portal backend or the old `services/portal` root wrapper remains as shell, facade, compatibility control plane or active verification owner. |
...
| `source:node-portal-backend-deployment-retirement` | Product + Operations | `services/portal/frontend/vite.config.ts`, `services/portal/frontend/src/api/**`, `services/medopl-go-backend` | `node tests/contracts/contract-test-v22-precloud-deployable-rc.mjs`; `npm --prefix services/portal/frontend run typecheck` | local pre-cloud source proof | Node Portal backend or the old `services/portal` root wrapper remains a deployable backend, frontend proxy target, typed API owner or current verification owner. |
