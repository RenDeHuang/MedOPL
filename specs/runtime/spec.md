...
| `runtime:local-rc-provider-key-message-backflow` | Integration + Runtime | `services/portal`, `services/opl-web-gateway`, `services/opl-runtime-bridge` | `node tests/local-rc/local-rc-test-v22-provider-key-message-backflow.mjs` with authorized `GFLABTOKEN` env | local RC authorized proof | Local RC evidence is production provider, real WebUI provider reply, real cloud, deploy or billing readiness. |
