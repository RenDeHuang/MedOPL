...
| `runtime:go-local-rc-parity` | Product + Runtime + Operations | `services/medopl-go-backend`, `services/portal/frontend/src/api/portal/**`, `services/portal/src`, `docs/runtime/README.md` | `node tests/contract/contract-test-v22-go-backend-service-surface.mjs`; `node tests/regression/portal/regression-test-v22-portal-runtime-real-api-data-closure.mjs`; `go test ./...` from `services/medopl-go-backend` | local deterministic RC proof | Go local RC parity is live provider, real OPL upstream, production secret storage, real cloud, deploy, kubectl, build/push, production billing or production runtime evidence. |
