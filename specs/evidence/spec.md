...
| `evidence:local-provider-bound-message-proof` | Runtime | `docs/evidence/README.md`, `docs/specs/README.md` | `node tests/local-rc/local-rc-test-v22-provider-bound-message-backflow.mjs` with authorized `GFLABTOKEN` env | local RC authorized proof | Local provider-bound proof is production provider, live external, deploy, real cloud or billing evidence. |
