...
assert.deepEqual(preSlideFastSuite.commands, [
  ...
  "node tests/governance/governance-test-v22-product-engineering-loop-index.mjs",
...
assert.deepEqual(localReleaseCandidateSuite.commands, [
  "node tests/governance/governance-test-v22-local-portal-opl-delivery-rc.mjs",
...
assert(reviewSuite?.commands.includes("node tests/governance/governance-test-v22-landing-closeout-automation.mjs"), "review_suite_must_check_landing_closeout");
assert(reviewSuite?.commands.includes("node tests/governance/governance-test-v22-root-verify-workflow-entrypoints.mjs"), "review_suite_must_check_root_verify_workflow");
assert(reviewSuite?.commands.includes("node tests/governance/governance-test-v22-framework-workflow-convergence.mjs"), "review_suite_must_check_framework_workflow");
assert(reviewSuite?.commands.includes("node tests/governance/governance-test-v22-product-engineering-loop-index.mjs"), "review_suite_must_check_product_engineering_loop");

const localContractSuite = manifest.suites.find((suite) => suite.id === "local-contract");
assert(localContractSuite?.commands.includes("node tests/governance/governance-test-v22-root-verify-workflow-entrypoints.mjs"), "local_contract_must_check_root_verify_workflow");
assert(localContractSuite?.commands.includes("node tests/governance/governance-test-v22-product-engineering-loop-index.mjs"), "local_contract_must_check_product_engineering_loop");
