...
const files = {
  ...
  packageDContract: "contracts/cloud-deploy-readiness-contract.json",
  productionLaunchContract: "contracts/cloud-authorization-boundary-contract.json",
};

const indexLoopGate = "node tests/governance/governance-test-v22-current-state-index-loop.mjs";
const landingCloseoutGate = "node tests/governance/governance-test-v22-landing-closeout-automation.mjs";
...
assert.equal(
  current.cloud_deploy_readiness_contract_ref?.owner_field,
  "cloud_deploy_readiness_contract",
  "package_d_contract_owner_field_mismatch",
);
assert.equal(
  current.cloud_authorization_boundary_contract_ref?.owner_field,
  "cloud_authorization_boundary_contract",
  "production_launch_contract_owner_field_mismatch",
);
assert.equal(
  packageDContract.schema_version,
  current.cloud_deploy_readiness_contract_ref?.schema_version,
  "package_d_contract_schema_mismatch",
);
assert.equal(
  productionLaunchContract.schema_version,
  current.cloud_authorization_boundary_contract_ref?.schema_version,
  "production_launch_contract_schema_mismatch",
);
assert.equal(
  Object.hasOwn(packageDContract, "cloud_deploy_readiness_contract"),
  true,
  "package_d_contract_payload_missing",
);
assert.equal(
  Object.hasOwn(productionLaunchContract, "cloud_authorization_boundary_contract"),
  true,
  "production_launch_contract_payload_missing",
);
assert.deepEqual(
  packageDContract.consumers,
  ["scripts/v22-verify.mjs", "tests/governance/governance-test-v22-current-state-index-loop.mjs"],
  "package_d_contract_consumers_mismatch",
);
assert.deepEqual(
  productionLaunchContract.consumers,
  ["scripts/v22-verify.mjs", "tests/governance/governance-test-v22-current-state-index-loop.mjs"],
  "production_launch_contract_consumers_mismatch",
);
assert(
  serializedLineCount(packageDContract) <= 160,
  `package_d_contract_must_remain_small:${serializedLineCount(packageDContract)}`,
);
assert(
  serializedLineCount(productionLaunchContract) <= 180,
  `production_launch_contract_must_remain_small:${serializedLineCount(productionLaunchContract)}`,
);
assert.deepEqual(
  packageDContract.cloud_deploy_readiness_contract.allowed_operation_classes,
  ["non_executing_shape_gate", "redacted_evidence_read", "future_authorized_deploy_runner"],
  "package_d_contract_allowed_ops_mismatch",
);
assert.deepEqual(
  productionLaunchContract.cloud_authorization_boundary_contract.evidence_sink.allowed_roots,
  [".runtime/"],
  "production_launch_contract_evidence_sink_mismatch",
);
assert.equal(
  productionLaunchContract.cloud_authorization_boundary_contract.authorization_boundary.default_real_cloud_execution,
  "forbidden_without_explicit_user_authorization",
  "production_launch_contract_authorization_boundary_mismatch",
);
...
assert(leaf.verification_commands.includes("node tests/governance/governance-test-v22-cleanup-lifecycle-system.mjs"), "current_leaf_must_run_lifecycle_gate");
