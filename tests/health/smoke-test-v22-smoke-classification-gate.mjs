...
async function listTestFiles(dir, prefix = "tests") {
  ...
  for (const entry of entries) {
    ...
    } else if (/^smoke-test-v22-.*\.mjs$/u.test(entry.name)) {
      files.push(repoPath);
    }
  }
  return files.sort();
...
const allSmokeScripts = await listTestFiles(testsRoot);
...
assert(defaultScripts.includes("tests/health/smoke-test-v22-smoke-classification-gate.mjs"), "default_suite_must_run_smoke_classification_gate");
assert(defaultScripts.includes("tests/health/smoke-test-v22-smoke-eval-boundary.mjs"), "default_suite_must_run_smoke_eval_boundary_gate");
assert(defaultScripts.includes("tests/contract/smoke-test-v22-golden-smoke-suite.mjs"), "default_suite_must_register_golden_smoke_suite");
assert(defaultScripts.includes("tests/health/smoke-test-v22-archive-smoke-contract-physical-retirement-gate.mjs"), "default_suite_must_run_archive_physical_retirement_gate");
...
].sort(), "suite_wrappers_must_be_explicit");
...
const mvpSuiteSource = await readFile(path.join(repoRoot, "tests/contract/smoke-test-v22-mvp-contract-suite.mjs"), "utf8");
