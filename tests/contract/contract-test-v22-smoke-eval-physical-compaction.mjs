...
const replacementAuthorities = Object.freeze({
  ...
  "scripts/check-one-person-lab-upstream-clean.mjs": "tests/contract/contract-test-v22-repo-governance-physical-compaction.mjs",
  "scripts/smoke-test-workspace-storage-routes-contract.mjs": "tests/regression/portal/regression-test-v22-workspace-storage-public-response.mjs",
});
...
const v22EvalFiles = Object.keys(SMOKE_CLASSIFICATION)
  .filter((scriptPath) => files.includes(scriptPath))
  .sort();
const classifiedTrackedScripts = Object.keys(SMOKE_CLASSIFICATION)
  .filter((scriptPath) => files.includes(scriptPath))
  .sort();
assert.deepEqual(classifiedTrackedScripts, v22EvalFiles, "all_tracked_v22_eval_files_must_be_classified");
...
assert.equal(v22EvalFiles.length, classifiedTrackedScripts.length, `v22_eval_count_mismatch:${v22EvalFiles.length}`);
assert(v22EvalFiles.length >= 148, `v22_tests_eval_count_must_not_drop_below_absorbed_baseline:${v22EvalFiles.length}`);
...
const compactionGate = "tests/contract/contract-test-v22-smoke-eval-physical-compaction.mjs";
...
const repoGovernanceGateText = await source("tests/contract/contract-test-v22-repo-governance-physical-compaction.mjs");
for (const scriptPath of retiredScripts) {
...
assert(override.commands.includes("node tests/contract/contract-test-v22-smoke-eval-physical-compaction.mjs"), "smoke_eval_physical_compaction_override_must_run_gate");
...
assert(localContract.commands.includes("node tests/contract/contract-test-v22-smoke-eval-physical-compaction.mjs"), "local_contract_suite_must_include_smoke_eval_compaction_gate");
...
console.log(JSON.stringify({
  ...
  v22EvalFiles: v22EvalFiles.length,
  tierCounts,
  retiredScripts,
  blockedCandidates,
}, null, 2));
