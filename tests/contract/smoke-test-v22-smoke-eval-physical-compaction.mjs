...
const replacementAuthorities = Object.freeze({
  ...
  "scripts/check-one-person-lab-upstream-clean.mjs": "tests/contract/smoke-test-v22-repo-governance-physical-compaction.mjs",
  "scripts/smoke-test-workspace-storage-routes-contract.mjs": "tests/regression/portal/smoke-test-v22-workspace-storage-public-response.mjs",
});
...
const v22SmokeFiles = files.filter((file) => /^tests\/.+\/smoke-test-v22-.+\.mjs$/u.test(file));
...
assert(v22SmokeFiles.length >= 148, `v22_tests_eval_count_must_not_drop_below_absorbed_baseline:${v22SmokeFiles.length}`);
...
const compactionGate = "tests/contract/smoke-test-v22-smoke-eval-physical-compaction.mjs";
...
const repoGovernanceGateText = await source("tests/contract/smoke-test-v22-repo-governance-physical-compaction.mjs");
for (const scriptPath of retiredScripts) {
...
assert(override.commands.includes("node tests/contract/smoke-test-v22-smoke-eval-physical-compaction.mjs"), "smoke_eval_physical_compaction_override_must_run_gate");
...
assert(localContract.commands.includes("node tests/contract/smoke-test-v22-smoke-eval-physical-compaction.mjs"), "local_contract_suite_must_include_smoke_eval_compaction_gate");
