...
const thisGate = "tests/contract/smoke-test-v22-tests-taxonomy-hard-retirement.mjs";
...
const testSmokeFiles = await listTestFiles(path.join(repoRoot, "tests"));
assert.equal(testSmokeFiles.length, 155, `tests_eval_file_count_mismatch:${testSmokeFiles.length}`);
assert.deepEqual(Object.keys(SMOKE_CLASSIFICATION).sort(), testSmokeFiles, "tests_eval_files_must_match_classification");
...
for (const scriptPath of testSmokeFiles) {
...
assert(override.commands.includes("node tests/health/smoke-test-v22-smoke-classification-gate.mjs"), "tests_taxonomy_override_must_run_classification_gate");
assert(override.commands.includes("node tests/health/smoke-test-v22-smoke-eval-boundary.mjs"), "tests_taxonomy_override_must_run_boundary_gate");
...
console.log(JSON.stringify({
  ...
  testsEvalFiles: testSmokeFiles.length,
  tierCounts,
  retiredEntrypoints: [
