...
const gatePath = "tests/contract/smoke-test-v22-product-goal-harness.mjs";
const executionOrderGatePath = "tests/contract/smoke-test-v22-product-goal-execution-order.mjs";
const consistencyGatePath = "tests/contract/smoke-test-v22-goal-state-consistency.mjs";
...
const branchScopedAllowedDiffPatterns = new Map([
  ...
  ["cleanup/v22-strict-monolith-zero-compat-active-surface", [
    ...
    "scripts/v22-tencent-authorized-resource-lifecycle-*.mjs",
    "scripts/smoke-test-no-legacy-billing-paths.mjs",
    "scripts/smoke-test-v11-cloud-status-ui-contract.mjs",
    "services/portal/src/app/*.mjs",
    "services/portal/src/config/portal-config.mjs",
    "services/portal/src/domain/*.mjs",
    "services/portal/src/integrations/billing-client.mjs",
    "services/portal/src/routes/*.mjs",
    "services/portal/src/state/*.mjs",
    "services/opl-web-gateway/src/*.mjs",
    "services/opl-runtime-bridge/src/*.mjs",
...
function assertOnlyAllowedFilesChanged(verifyManifest = {}) {
  ...
  const branchAllowedPatterns = (branchScopedAllowedDiffPatterns.get(branchName) ?? []).map(globToRegExp);
...
function diffAddedLinesFromBase() {
  const diffArgs = currentBranchName() === "cleanup/v22-strict-monolith-ideal-gap-and-legacy-retirement"
    ? ["diff", "--unified=0", "--diff-filter=ACMR", "origin/recovery/platform-v22-trunk"]
    : ["diff", "--unified=0", "origin/recovery/platform-v22-trunk"];
