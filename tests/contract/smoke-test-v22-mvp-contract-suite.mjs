...
const branchOverrideSuiteIds = new Set([
  ...
  "tests-taxonomy-hard-retirement",
]);
...
const smokeScripts = listClassifiedSmokeScripts({ categories: DEFAULT_SMOKE_CATEGORIES })
  .filter((scriptPath) => scriptPath !== "tests/contract/smoke-test-v22-mvp-contract-suite.mjs")
...
async function runBranchOverrideVerify(expectedSuiteId) {
  ...
  assert.equal(
    payload.commands.includes("node tests/regression/portal/smoke-test-v22-portal-ui-design-quality-audit.mjs"),
    false,
