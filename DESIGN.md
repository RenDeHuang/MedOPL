...
后续 UI 重构不能只看“像不像大厂”或截图是否稳定。每个页面、组件和 Figma 方案都必须按同一套量表评审：

- 产品真相：是否仍然是资源购买与计算资源管理 Portal，没有滑向 chat、科研工作台、云控制台或 Trace 控制台。
- 用户任务：是否直接服务买资源、开通计算资源、查看存储、查看费用、扩容、释放、停止计费确认或进入 OPL。
- 信息层级：首屏是否先回答资源能不能用、费用是否正常、下一步做什么，而不是 KPI 墙或工程字段。
- 视觉偏倚：是否避免默认 shadcn / Tailwind / Vercel 风、深色 admin 后台、卡片墙和蓝紫科技模板。
- 组件语法：是否复用合同中的核心组件语法；新增组件必须有明确任务和 owner。
- 状态覆盖：loading、empty、ready、blocked、failed、pending、released、protected 是否有清楚表达和恢复路径。
- 交互反馈：主动作、危险动作、禁用态、释放确认和进入 OPL readiness 是否有明确反馈。
- 可访问性：键盘、focus-visible、触控目标、禁用原因、非纯颜色状态、导航 landmark 和语义标题是否成立。
- 响应式行为：移动端无横向溢出；表格必须折叠为可读结构或安全容器。
- 文案语言：用户可见文案必须是中文资源控制面语言，不能泄露工程状态词。

设计评审必须全部维度通过，才能进入实现或回归；即便全部通过，也不能单独 claim production ready。

## 固定与弹性

UI contract 固定原则和验收，不固定像素。
...
- 可访问性底线。
- 生产 claim 边界。
- raw Figma、截图和调试 evidence 不进 git。
...
页面级 override 必须说明它服务哪个用户任务，不能突破产品禁区，不能新增顶层 script、无 owner contract 或治理型 health test。这个规则的目的不是放松质量，而是避免把资源控制台固定成僵硬模板。

## 协同设计闭环

Figma、AI 设计和前端实现必须按同一条闭环推进：
...
高保真不是起点。宽泛 prompt 或一次性截图容易产生拼贴式混乱布局、重复元素、信息流混乱和默认组件库味。MedOPL 的做法是先用 contract 约束产品对象、状态和禁区，再允许 Figma / AI 在页面布局和视觉表达上探索。

协同设计输出只允许进入现有 truth 面：`DESIGN.md`、`contracts/medopl-portal-page-state-matrix.json`、Portal source、fixtures 或现有 tests。不得因为一次设计探索新增顶层 script、长期 docs 文件、raw Figma 导出图或截图证据。
