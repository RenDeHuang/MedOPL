...
- [x] Step 2: define local provider-key-bound evidence boundary
- [x] Step 3: add local RC provider-key-bound message backflow eval
- [x] Step 4: fix implementation only if the eval exposes a code bug
- [x] Step 5: run authorized local RC with gflabtoken from `~/.secrets/medopl/secrets.env.txt`
...
- Step 3 added `tests/local-rc/local-rc-test-v22-provider-key-message-backflow.mjs`, `local-rc-authorized` lane metadata and evidence/spec links.
...
- Step 5 ran with user-authorized `GFLABTOKEN` extracted from `~/.secrets/medopl/secrets.env.txt` without printing the value.
