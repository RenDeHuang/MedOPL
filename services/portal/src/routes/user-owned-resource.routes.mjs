import { createPortalUserOwnedResourceStore } from "../state/portal-user-owned-resource-store.mjs";
...
const RESOURCE_PATHS = {
  list: ["/portal/api/platform-provisioned-resources", "/portal/api/user-owned-resources"],
  computeInstances: ["/portal/api/platform-provisioned-resources/compute-instances", "/portal/api/user-owned-resources/compute-instances"],
  storageBuckets: ["/portal/api/platform-provisioned-resources/storage-buckets", "/portal/api/user-owned-resources/storage-buckets"],
  deleteCompute: ["/portal/api/platform-provisioned-resources/compute-instances/delete", "/portal/api/user-owned-resources/compute-instances/delete"],
  deleteStorage: ["/portal/api/platform-provisioned-resources/storage-buckets/delete", "/portal/api/user-owned-resources/storage-buckets/delete"],
  bind: ["/portal/api/platform-provisioned-resources/bind", "/portal/api/user-owned-resources/bind"],
  unbind: ["/portal/api/platform-provisioned-resources/unbind", "/portal/api/user-owned-resources/unbind"],
  protectionFreezes: ["/portal/api/platform-provisioned-resources/protection-freezes", "/portal/api/user-owned-resources/protection-freezes"],
  ensureProtectionFreeze: ["/portal/api/platform-provisioned-resources/protection-freezes/ensure", "/portal/api/user-owned-resources/protection-freezes/ensure"],
};

function resourcePath(url, key = "list") {
  return (RESOURCE_PATHS[key] || []).includes(String(url?.pathname || ""));
}

export function createUserOwnedResourceRoutes({ readBody, sendJson, writeDb, cloudProvisioner = null }) {
  const store = createPortalUserOwnedResourceStore({ writeDb, cloudProvisioner });
  ...
  async function handleList({ req, res, url, db, user }) {
    if (req.method !== "GET" || !resourcePath(url)) return false;
    sendJson(res, store.listOwnerScopedResources(db, user));
    return true;
  }

  async function handleCreateCompute({ req, res, url, db, user }) {
    if (req.method !== "POST" || !resourcePath(url, "computeInstances")) return false;
  ...
  async function handleCreateBucket({ req, res, url, db, user }) {
    if (req.method !== "POST" || !resourcePath(url, "storageBuckets")) return false;
    return safeAction(res, async () => {
      const payload = parseJsonBodyOrEmpty(await readBody(req));
  ...
  async function handleDeleteCompute({ req, res, url, db, user }) {
    if (req.method !== "POST" || !resourcePath(url, "deleteCompute")) return false;
    return safeAction(res, async () => {
      const payload = parseJsonBodyOrEmpty(await readBody(req));
  ...
  async function handleDeleteBucket({ req, res, url, db, user }) {
    if (req.method !== "POST" || !resourcePath(url, "deleteStorage")) return false;
    return safeAction(res, async () => {
      const payload = parseJsonBodyOrEmpty(await readBody(req));
  ...
  async function handleBind({ req, res, url, db, user }) {
    if (req.method !== "POST" || !resourcePath(url, "bind")) return false;
    return safeAction(res, async () => {
      const payload = parseJsonBodyOrEmpty(await readBody(req));
  ...
  async function handleUnbind({ req, res, url, db, user }) {
    if (req.method !== "POST" || !resourcePath(url, "unbind")) return false;
    return safeAction(res, async () => {
      const payload = parseJsonBodyOrEmpty(await readBody(req));
  ...
  async function handleProtectionFreezeList({ req, res, url, db, user }) {
    if (req.method !== "GET" || !resourcePath(url, "protectionFreezes")) return false;
  ...
  async function handleProtectionFreezeEnsure({ req, res, url, db, user }) {
    if (req.method !== "POST" || !resourcePath(url, "ensureProtectionFreeze")) return false;
    return safeAction(res, async () => {
      const payload = parseJsonBodyOrEmpty(await readBody(req));
  ...
  return async function handleUserOwnedResourceRoutes(context) {
