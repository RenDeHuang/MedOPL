...
const componentRegistry: SurfaceEntry[] = [
  {
    ...
    props: (context) => ({
      ...
      computeSummary: context.restricted ? "待选择套餐" : "2 核 / 4GB",
      entitlementStatus: "可用",
      nextActionDetail: context.restricted ? "先确认套餐、余额和托管运行环境状态。" : "环境、套餐、余额和文件空间均可用，可以进入 OPL 运行任务。",
      nextActionHref: context.restricted ? "/resources" : "/opl-launch",
      nextActionLabel: context.restricted ? "查看受限原因" : "进入 OPL 工作台",
      releaseStatus: context.restricted ? "未开通" : "停止计费待确认 / T+1 可审计",
      selectedPlanName: context.restricted ? "未选择" : "基础套餐",
      serverPlansReady: !context.loading,
      serviceSummary: context.restricted ? "未选择套餐托管科研工作台服务" : "基础套餐托管科研工作台服务",
      storageSummary: context.restricted ? "待开通文件空间" : "10 GB 文件空间",
    }),
  },
  {
  ...
  },
  {
    ...
    props: (context) => ({
      ...
      money: yuan,
      releaseStatus: context.empty ? "未开通" : "停止计费待确认 / T+1 可审计",
      statusBadge,
    }),
  },
  {
  ...
  },
  {
    ...
    props: (context) => ({
      computeSummary: context.empty ? "待选择套餐" : "2 核 / 4GB",
      ...
      storageSummary: context.empty ? "待开通文件空间" : "10 GB 文件空间",
    }),
  },
  {
